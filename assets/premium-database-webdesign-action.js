(function (global) {
    ...
    function createController(options) {
        ...
        function hasApprovedMockup(customer) {
            return Boolean(customer && isValidWebsitePhotoDataUrl(customer.websiteMockup));
        }
        ...
        function render(customer) {
            ...
            const hasPhoto = assetState ? assetState.hasPhoto : isValidWebsitePhotoDataUrl(photo), hasPhotoSource = Boolean(photo);
            ...
            const photoLoaded = !hasPhoto || !hasPhotoSource || isInlinePhotoSource(photo) || loadedPhotoKeys.has(photoLoadKey);
            const photoFailed = hasPhoto && hasPhotoSource && failedPhotoKeys.has(photoLoadKey);
            ...
            const hasMockup = assetState ? assetState.hasMockup : isValidWebsitePhotoDataUrl(mockup), hasMockupSource = Boolean(mockup);
            ...
            const mockupLoaded = !hasMockup || !hasMockupSource || isInlinePhotoSource(mockup) || loadedPhotoKeys.has(mockupLoadKey);
            const mockupImageFailed = hasMockup && hasMockupSource && failedPhotoKeys.has(mockupLoadKey);
            ...
            const mockupInner = hasMockup
                ? (!hasMockupSource || mockupImageFailed ? FALLBACK_ICON : "<span class=\"photo-drop-loader\" aria-hidden=\"true\">" + LOADING_ICON + "</span><img class=\"photo-drop-image\" src=\"" + escapeHtml(mockup) + "\" alt=\"" + escapeHtml(mockupLabel) + "\" loading=\"lazy\" fetchpriority=\"low\" decoding=\"async\" width=\"34\" height=\"34\">")
                : (mockupLoading ? LOADING_ICON : MOCKUP_ICON);
            const mockupAriaLabel = hasMockup ? (!hasMockupSource ? "Device mockup beschikbaar; detaildata laadt op de achtergrond" : (mockupImageFailed ? "Device mockup kon niet geladen worden" : (canGenerateMockup ? "Device mockup opnieuw maken" : "Device mockup bekijken"))) : (mockupLoading ? "Device mockup wordt gemaakt" : (mockupGenerationFailed ? "Device mockup maken is mislukt" : (canGenerateMockup ? "Device mockup maken" : "Device mockup nog niet beschikbaar")));
        ...
        async function startJobForTarget(target, startOptions) {
            ...
            try {
                ...
                if (job.status === "error") {
                    const errorMessage = job.safetyBlocked ? "De AI-aanbieder heeft dit ontwerp geblokkeerd. Er is geen ontwerp opgeslagen." : (normalizeString(job.error) || "Webdesign maken is mislukt.");
        ...
        return {
            ...
            render: render,
            resumePendingJobs: resumePendingJobs, startBulkBatchForCustomers: startBulkBatchForCustomers
        };
    }
    ...
    function createOutreachController(options) {
        ...
        function renderMeta(customer, forceOutreachMeta) {
            if (isInstantlyConfirmedSent(customer)) {
                return customer.instantlyManualSentClassification === "user-requested-unverified"
