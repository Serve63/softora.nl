    ...
    async function run(options) {
        ...
        return (async function () {
            try {
                ...
                global.performance?.mark?.("softora:database:payloads-ready"); if (!snapshotClient.markCanonicalInventoryReady(state)) throw new Error("Volledige databasevoorraad is niet geladen."); renderPage(); global.performance?.mark?.("softora:database:first-render"); await webdesignActionController.preloadPhotoImages(getSortedCustomers(getFilteredCustomers()), 16, 1200); global.performance?.mark?.("softora:database:photos-preloaded"); state.photoRestorePending = false; renderPage(); global.performance?.mark?.("softora:database:final-render"); if (options.databaseReadiness) await options.databaseReadiness.publish({ state }); releaseDatabaseBootShell(); void databasePendingJobsPromise.catch(function (error) { console.error("Websitefoto jobs hervatten mislukt:", error); });
