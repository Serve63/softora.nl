    ...
    async function run(options) {
        ...
        return (async function () {
            try {
                ...
                if (!snapshotClient.markCanonicalInventoryReady(state)) throw new Error("Volledige databasevoorraad is niet geladen."); renderPage(); await webdesignActionController.preloadPhotoImages(getSortedCustomers(getFilteredCustomers()), 16, 1200); state.photoRestorePending = false; renderPage(); if (options.databaseReadiness) await options.databaseReadiness.publish({ state }); releaseDatabaseBootShell(); void databasePendingJobsPromise.catch(function (error) { console.error("Websitefoto jobs hervatten mislukt:", error); });
