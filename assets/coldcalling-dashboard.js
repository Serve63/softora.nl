(function () {
  ...
  function getDialableLeadsForRegioCount() {
    const parsed = parseLeadRows(getSavedLeadRows());
    if (!parsed.leads.length) return [];
    const blocked = getDoNotCallPhoneKeys();
    return parsed.leads.filter((lead) => {
      const key = phoneKey(lead?.phone);
      if (!key) return true;
  ...
  function paintRegioLeadCountOnCustomSelectValue() {
    ...
    if (!valueEl) return;
    const selectedOption = regioSel.options[regioSel.selectedIndex] || regioSel.options[0];
    const baseLabel = selectedOption ? String(selectedOption.textContent || '').trim() : '';
  ...
  function hookRegioLeadCountCustomSelectSync() {
    const regioSel = byId('regio');
    if (!regioSel || regioSel.dataset.regioLeadCountHook === '1') return;
    if (typeof regioSel.__softoraSyncCustomFormSelect !== 'function') return;
    regioSel.dataset.regioLeadCountHook = '1';
    const originalSync = regioSel.__softoraSyncCustomFormSelect;
    regioSel.__softoraSyncCustomFormSelect = function regioSyncWithLeadCount() {
      originalSync.call(regioSel);
