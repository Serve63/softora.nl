...
(function (root, factory) {
  ...
  function buildAppJs() {
    return [
      ...
      '      e.preventDefault(); if(form.checkValidity && !form.checkValidity()) return;',
      '      var control = e.submitter || form.querySelector("[data-softora-whatsapp-action=\\"submit\\"]"), msg = form.querySelector(".form-feedback");',
      '      if(control && String(control.getAttribute("data-softora-conversion-target") || "").trim() === "whatsapp" && String(control.getAttribute("data-softora-whatsapp-url") || "").trim() === "https://wa.me/31643262792"){ var eventData = { name: String(control.getAttribute("data-softora-conversion") || "").trim(), page: String(control.getAttribute("data-softora-conversion-page") || "contact.html").trim(), target: "whatsapp", path: String(window.location && window.location.pathname || "contact.html") + String(window.location && window.location.search || ""), at: new Date().toISOString() }; window.__softoraPublicConversionEvents = window.__softoraPublicConversionEvents || []; window.__softoraPublicConversionEvents.push(eventData); window.__softoraPublicLastConversion = eventData; try { window.dispatchEvent(new CustomEvent("softora:public-conversion", { detail: eventData })); } catch(err) {} window.open("https://wa.me/31643262792", "_blank", "noopener,noreferrer"); }',
