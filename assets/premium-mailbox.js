...
function renderMailboxParagraphs(lines, options) {
  if (options?.aiManaged) return window.SoftoraMailboxAiPresentation.renderBody(lines);
  const renderedLines = [];
...
function renderMailboxBodySection(section, imageState, leadingHtml = '', options = {}) {
  return window.SoftoraMailboxBodySection.render(section, imageState, leadingHtml, {
    ...options,
    escapeHtml,
    isReplyHeaderLine: isMailboxReplyHeaderLine,
    isOwnReplyHeaderLine: isMailboxOwnReplyHeaderLine,
    ownReplyAuthorPattern: MAILBOX_OWN_REPLY_AUTHOR_PATTERN,
    trimQuotedLines: window.SoftoraMailboxDisplay.trimOwnQuotedMailLines,
    prepareOwnQuote: window.SoftoraMailboxImages.prepareOwnQuote,
    renderInlineImage: renderMailboxInlineImage,
    renderParagraphs: renderMailboxParagraphs,
  });
}
...
function renderMailboxConversationAction(action, mailId, options = {}) {
  ...
  const command = isNewMessage ? 'new-message' : 'reply-mail';
  const label = isNewMessage ? 'Nieuw bericht sturen' : 'Beantwoorden';
...
function renderMailBody(value, images, options) {
  ...
  const rootPresentation = window.SoftoraMailboxCampaignInbox.getRootMessagePresentation(value, options && options.mail); imageState.aiManaged = rootPresentation.aiManaged; const sections = (rootPresentation.aiManaged ? [{ type: 'body', lines: rootPresentation.body.split('\n') }] : buildMailboxBodySections(rootPresentation.body)).filter((section) => (
  ...
  const rootSentStart = window.SoftoraMailboxDiscovery?.renderRootSentCardStart?.(options && options.mail, { contactDossierMode, isProvenMailboxCopy, rootIncoming, campaignInbox: window.SoftoraMailboxCampaignInbox, escapeHtml }) || '';
  ...
  if (rootSentStart) renderedSections.push(rootSentStart);
  sections.forEach((section) => {
    if (rootIncoming && rootIncomingOpen && section && section.type === 'quote') {
    ...
    renderedSections.push(renderMailboxBodySection(section, imageState, sectionLead, {
      embeddedIncoming: rootIncoming && rootIncomingOpen,
      rootIncomingQuote: rootIncoming && !rootIncomingOpen && section && section.type === 'quote',
    }));
  ...
  if (rootIncomingOpen && !insertedReplyAction && rootActionHtml && !isProvenMailboxCopy) {
    renderedSections.push(rootActionHtml);
    insertedReplyAction = true;
  }
  if (rootIncomingOpen) renderedSections.push('</section>');
  if (!insertedReplyAction && rootActionHtml && !isProvenMailboxCopy) {
    renderedSections.push(rootActionHtml);
    if (!isProvenMailboxCopy && olderThreadMessagesHtml) {
      renderedSections.push(olderThreadMessagesHtml);
  ...
  if (rootSentStart && imageState.quoteImages.length) renderedSections.push(window.SoftoraMailboxImages.renderOwnQuoteSection(imageState.quoteImages.splice(0), imageState, renderMailboxInlineImage));
  if (rootSentStart) renderedSections.push('</section>');
...
let mails = [];
let activeFolder = 'outreach';
let activeMail = null;
let inboxUnreadCount = 0;
let mailboxSyncState = null;
let mailboxOwnerView = null, mailboxRefreshController = null, mailboxDiscoveryController = null, mailboxDetailController = null;
...
const mailboxReadController = window.SoftoraMailboxRead.create({
  ...
  onExternalState() { window.SoftoraMailboxUiState.refresh({ mails, controller: mailboxReadController, renderList, getActiveMail: () => activeMail, openMail }); },
  toast,
});
...
const mailboxComposeController = window.SoftoraMailboxComposeController.create({
  ...
  campaignInbox: window.SoftoraMailboxCampaignInbox,
...
function normalizeMailboxApiMessage(message, options = {}) {
  const when = formatMailDate(message.receivedAt || message.date);
  const body = message.aiPresentation ? String(message.body || '') : cleanMailboxText(message.body || '');
  ...
  const indexedMail = window.SoftoraMailboxIndex && typeof window.SoftoraMailboxIndex.decorateMessage === 'function'
    ? window.SoftoraMailboxIndex.decorateMessage(decoratedMail, message)
    : decoratedMail;
  return window.SoftoraMailboxUiState.reconcileMessage(indexedMail, { skipAccepted: options.skipAcceptedReconcile, composeController: mailboxComposeController, readController: mailboxReadController });
}
...
async function loadMailboxMessageBody(id, options = {}) {
  ...
  const token = options.token || mailboxOwnerView.getToken();
  if (!isMailboxViewCurrent(token)) return;
  await window.SoftoraMailboxIndex.loadBody({
    id, requestId: window.SoftoraMailboxCampaignInbox.getRequestId(mail),
    ...
    openMail: typeof options.openMail === 'function' ? options.openMail : openMail,
    isCurrent: () => isMailboxViewCurrent(token),
    signal: token.signal,
  });
}
function retryMailboxThreadMessage(id, messageKey) {
  ...
  void openMail(mail.id, { skipBodyFetch: true, skipContactTimeline: true, skipReadPersist: true });
}
function renderMailboxDetailHtml(m) {
...
function openMail(id, options = {}) {
  return mailboxDetailController?.open?.(id, options) || Promise.resolve({ committed: false, stale: true });
}
  ...
  await mailboxDeleteController.remove(m, {
    ...
    commit(_mail, transaction) { mailboxLogicalDeleteBridge.commit();
      toast('Gesprek alleen in Softora verborgen', {
        ...
        async action() {
          const result = await mailboxDeleteController.restore(m);
          if (!result.ok) return;
          mails = transaction?.mails || mails;
          activeMail = transaction?.activeMail || m.id;
          renderList({ openLatest: false });
          if (activeMail) openMail(activeMail, { skipBodyFetch: true });
          else resetDetailEmpty();
          toast('Gesprek hersteld in Softora');
        },
      });
    },
  });
...
function handleMailboxAction(actionEl) {
  ...
  switch (action) {
    case 'set-folder':
      ...
      break;
    case 'open-mail':
      if (!mailboxDiscoveryController?.isSearchActive?.() || mailboxDiscoveryController.canOpenResult?.(findMailById(id))) openMail(id);
      break;
    case 'delete-mail':
      void deleteMail(id);
      break;
...
mailboxRefreshController = window.SoftoraMailboxRefresh?.create({ autoStart: false, initiallyChecking: false, getAccount: () => activeMailboxAccount, getFolder: () => activeFolder, getOwner: () => window.SoftoraMailboxCampaignInbox.getOwner(), getLastSnapshotAt: () => Date.parse(mailboxSyncState?.snapshotSavedAt || '') || 0, loadMessages: loadMailboxMessages, readStored: ({ signal }) => loadMailboxMessages({ signal, showLoader: false, silentRead: true, skipBackgroundSync: true, skipProviderRefresh: true, skipPageBootstrap: true, preferSnapshot: true, openLatest: false, preserveOnError: true, reuseActiveToken: true }), toast });
  ...
  mailboxAccountMenu.addEventListener('click', function(event) {
    ...
    if (ownerButton) {
      ...
      if (pinOwner) {
        event.preventDefault();
        event.stopPropagation();
        void window.SoftoraMailboxCampaignInbox
          .pinOwner(selectedOwner, window.SoftoraUiStateClient)
          .then((result) => {
            if (window.SoftoraMailboxCampaignInbox.getOwner() !== selectedOwner) return;
            renderMailboxAccountMenu();
            toast(result.saved
              ? `Mailbox vastgepind: ${result.label}`
              : `Mailbox gekozen: ${result.label}. Vastpinnen opslaan mislukt.`);
          });
      }
      return;
    }
