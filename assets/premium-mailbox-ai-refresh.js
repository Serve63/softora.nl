(function (global) {
  ...
  function create({ getMail, getActiveId, getOwner, openMail, fetchImpl = global.fetch,
    ...
    let timer = null, flight = false, generation = 0, target = null, targetOwner = null;
    function watch(mail) {
      if (!mail) return;
      ...
      const pending = [mail, ...(mail.threadMessages || [])].filter((m) => m.aiPresentation?.status === 'pending').slice(0, 20);
    ...
    function stop() { generation += 1; if (timer) cancel(timer); timer = null; }
    return { watch, stop };
  }
  const api = { create };
