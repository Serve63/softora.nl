(function (global) {
    ...
    function ensureStyles() {
        ...
        style.textContent = ".mass-research-modal{width:min(760px,calc(100vw - 24px));max-height:calc(100vh - 34px);overflow:auto}.mass-research-grid{display:grid;gap:12px}.mass-research-row{display:grid;grid-template-columns:1fr 150px;gap:10px}.mass-research-row input,.mass-research-grid textarea{width:100%}.mass-research-grid textarea{min-height:136px;resize:vertical}.mass-research-actions{display:flex;gap:10px;flex-wrap:wrap}.mass-research-preflight{border:1px solid var(--line);border-radius:8px;padding:10px;background:#fff;color:var(--muted);font-size:13px}.mass-research-preflight.ready{border-color:#b7e4ca;background:#f3fff8;color:#166534}.mass-research-preflight.blocked{border-color:#fecaca;background:#fff5f5;color:#991b1b}.mass-research-stats{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px}.mass-research-stat{border:1px solid var(--line);border-radius:8px;padding:10px;background:#fff}.mass-research-stat strong{display:block;font-size:18px;color:var(--dark)}.mass-research-stat span{font-size:11px;font-weight:800;color:var(--muted);text-transform:uppercase}.mass-research-status{min-height:22px;font-size:13px;color:var(--muted)}@media(max-width:720px){.mass-research-row,.mass-research-stats{grid-template-columns:1fr}}";
    ...
    function createController() {
        ...
        const nodes = {
            ...
            reload: $("#massResearchReload", modal), stats: $("#massResearchStats", modal), status: $("#massResearchStatus", modal), preflight: $("#massResearchPreflight", modal)
        };
        let currentJobId = "";
        let running = false;
        ...
        const setRunning = (value) => {
            ...
            nodes.start.disabled = running || startBlocked;
            nodes.cancel.disabled = !running || !currentJobId;
        };
        const renderPreflight = (status) => {
            const googleReady = Boolean(status && status.googlePlacesConfigured);
            const parallel = status && status.limits ? status.limits.enrichmentConcurrencyDefault : 50;
            startBlocked = !googleReady;
            nodes.preflight.className = "mass-research-preflight " + (googleReady ? "ready" : "blocked");
            nodes.preflight.textContent = googleReady
                ? "Startklaar: Google Places staat aan, parallel " + parallel + ", partial upsert en duplicate-poort actief."
                : "Niet startklaar: Google Places server-key ontbreekt. Voeg GOOGLE_MAPS_SERVER_API_KEY toe in productie.";
            setRunning(running);
        };
        const loadPreflight = () => {
            nodes.preflight.className = "mass-research-preflight";
            nodes.preflight.textContent = "Startcheck laden...";
            return requestJson("/api/premium-database/mass-research-jobs/status")
                .then(renderPreflight)
                .catch((error) => {
                    startBlocked = false;
                    nodes.preflight.className = "mass-research-preflight blocked";
                    nodes.preflight.textContent = "Startcheck mislukt: " + (error.message || "onbekende fout") + ".";
                    setRunning(running);
                });
        };
        ...
        function start() {
            if (startBlocked) return setStatus("Start geblokkeerd tot Google Places is ingesteld.");
