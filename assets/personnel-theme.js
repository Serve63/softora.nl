...
`;
    ...
    function renderSidebarLink(link, activeKey) { return window.SoftoraPremiumSidebarLinks.renderSidebarLink(link, activeKey); }
    function getCustomersSidebarLink() { return window.SoftoraPremiumSidebarLinks.getCustomersSidebarLink(); }
    function getDatabaseSidebarLink() { return window.SoftoraPremiumSidebarLinks.getDatabaseSidebarLink(); }
    /** Zijbalkitems die achter een toegangsslot / coming-soon scherm zitten */
    const PREMIUM_SIDEBAR_COMING_SOON_KEYS = new Set(window.SoftoraPremiumSidebarLinks.COMING_SOON_KEYS);
    const COMING_SOON_LOCK_SVG = window.SoftoraPremiumSidebarLinks.COMING_SOON_LOCK_SVG;
    function getMailboxSidebarLink() { return window.SoftoraPremiumSidebarLinks.getMailboxSidebarLink(); }
    function activateMailboxSidebarLink(sidebar) { const target = sidebar || document.querySelector(".sidebar"); const link = target && target.querySelector('[data-sidebar-key="mailbox"]'); if (!link) return; const mailboxLink = getMailboxSidebarLink(); link.setAttribute("href", mailboxLink.href); link.classList.remove("sidebar-link--coming-soon"); link.removeAttribute("aria-disabled"); link.removeAttribute("tabindex"); link.querySelectorAll(".sidebar-link-lock").forEach(function (lock) { lock.remove(); }); if (!link.querySelector(":scope > .sidebar-link-mailbox-icon")) { link.querySelectorAll(":scope > svg").forEach(function (icon) { icon.remove(); }); link.insertAdjacentHTML("afterbegin", mailboxLink.icon); } const label = link.querySelector(".sidebar-link-text"); if (label) label.textContent = mailboxLink.label; }
    ...
    function getPremiumSidebarAdminExtraLinks() { return window.SoftoraPremiumSidebarLinks.getPremiumSidebarAdminExtraLinks(); }
    ...
    function buildUnifiedPremiumSidebarHtml(activeKey) {
        const [overviewLinks, managementLinks, adsLinks, socialLinks, extraLinks] =
            window.SoftoraPremiumSidebarLinks.getPremiumSidebarSections(premiumSessionSnapshot).map(section => section.links);
