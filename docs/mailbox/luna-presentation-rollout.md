...
2026-09-23 scope correction: the owner's “all mail” means only conversations admitted to the Softora coldmailbox, not every indexed Gmail message. The broad index was mistakenly described as the coldmailbox. `20260923122053_mailbox_ai_campaign_only.sql` now applies the existing durable per-message campaign proof to candidate discovery, claims, and pending-message gating. Already-paid decisions and uncertain cost holds are retained. `20260923122633_mailbox_ai_campaign_hint.sql` adds an indexed quick prefilter before that full proof to keep candidate and claim RPCs below their five-second deadlines. On production, the candidate query returned 20 rows in 3.36 seconds and one claim in 1.84 seconds under a five-second statement limit; both probes were rolled back. A later audit found every newly claimed sample had campaign proof. The $18 USD budget ceiling was restored after the gate was live; it was temporarily set to zero while the broad worker was stopped.
