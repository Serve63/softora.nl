...
De ene bestaande automation bedient nu Softora en de negen academies uit `docs/growth/seo-machine-sites.json`; de uitvoering per site en hervatbare wachtrij staan in `docs/growth/seo-machine-portfolio.md`. De Softora-specifieke regels hieronder blijven voor Softora bindend. Iedere academie heeft eigen content, aanbod, onderzoek en resultaatregistratie. Lokale academies leveren `local_ready` en tellen niet als live publicatie. Een dagelijkse portfolioronde behandelt alle tien sites achtereenvolgens; een blokkade blijft bij de betreffende site. De limieten van zeven nieuwe URL's en twee geldpagina's gelden per site.
