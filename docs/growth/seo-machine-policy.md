...
- Voer drie onderdelen in dezelfde run uit: het volledige operationele controlepad, een nieuwe publieke groeipagina en een begrensde ondersteunende optimalisatie op bestaande content.
...
- Nieuwe publieke groeipagina: lever per succesvolle niet-P0-run precies een nieuwe indexeerbare `editorial`- of `money_page`-URL op wanneer de live 7-daagse target nog niet is gevuld. De geplande dagelijkse heartbeat stuurt zo op exact maximaal zeven nieuwe groei-URL's per rollende 7 dagen; een achtste wijst op dubbele of buiten-procespublicatie en is een operationele P0 totdat de ledger en oorzaak zijn verklaard.
- Ondersteunende optimalisatie: voer naast de nieuwe URL de actie uit die bij de machine-toestand hoort, zoals query/pagina-match, indexatie/discovery, unieke informatiewinst, snippet, conversie of contextuele interne links. Iedere nieuwe URL krijgt in dezelfde gerichte wijziging minimaal een natuurlijke inkomende link vanaf een relevante bestaande pagina; het selectiebewijs benoemt die bestaande pagina en de onderbouwing.
- De `money_page`-lane omvat nieuwe diensten-, branche- en regiolandingspagina's en heeft een harde cap van maximaal twee live nieuwe URL's per rollende 7 dagen. De `editorial`-lane omvat blogs, kennisbankartikelen en vergelijkingen, vult minimaal de overige vijf dagen en heeft geen aparte lage weekcap; het ritme van een succesvolle dagelijkse heartbeat blijft de normale begrenzing.
...
- Houd in iedere niet-P0-toestand zeven nieuwe groei-URL's per rollende 7 dagen als dagelijkse target: minimaal vijf `editorial`, maximaal twee `money_page`. Een geldpagina neemt nooit de plaats in van een gemiste redactionele ondergrens.
- Als de target is gemist, publiceert de eerstvolgende veilige run de hoogst scorende unieke kandidaat uit de door cadence toegestane lane. Alleen een echte operationele P0, aantoonbare cannibalisatie van alle veilige kandidaten of een externe merge/deployblokkade mag dit tegenhouden.
...
| `data_degraded` | GSC- of URL Inspection-data ontbreekt of is onvoldoende betrouwbaar | Meting repareren en alleen een eerder bewijsdekte veilige kandidaat gebruiken | Dagelijkse target blijft 7; money-pagecap blijft 2 |
| `indexation_recovery` | Minimaal vijf D14/D28-URL's zijn inspecteerbaar en minder dan 60% is geïndexeerd | Discovery, canonicals, consolidatie en contextuele links verbeteren | Dagelijkse target blijft 7; money-pagecap blijft 2 |
| `performance_recovery` | Minimaal vijf D28-URL's zijn reviewbaar en minder dan 40% krijgt non-branded impressies, of de cohort heeft minstens 100 impressies zonder klik | Query/pagina-match, snippets, interne routes en positionering verbeteren | Dagelijkse target blijft 7; money-pagecap blijft 2 |
| `quality_recovery` | Templateaandeel, herhaalde paragrafen of dichtstbijzijnde pagina-overlap overschrijdt de interne kwaliteitsgrens | Unieke informatiewinst toevoegen en overlap consolideren | Dagelijkse target blijft 7; money-pagecap blijft 2 |
| `growth` | Techniek en herstelpoorten zijn groen | Hoogste verwachte gekwalificeerde impact en bestaande clusterroute versterken | Dagelijkse target blijft 7; money-pagecap blijft 2 |
| `scale` | Minimaal vijf reviewbare URL's, minstens 80% D14/D28-indexatie, minstens 60% D28-dekking met non-branded impressies, ten minste een non-branded klik en groene kwaliteit | Bewezen clusters gecontroleerd uitbouwen | Dagelijkse target blijft 7; money-pagecap blijft 2 |
...
`performance_recovery` is geen publicatiestop en verlaagt de redactionele lane niet. Zolang overlap, claims en kandidaatkwaliteit groen zijn, publiceert de machine dagelijks een unieke veilige groei-URL en gebruikt zij de verplichte begeleidende actie voor bestaande query/pagina-matches, snippets en interne routes.

`npm run seo:publications:report` rapporteert daarom afzonderlijk `newUrls`, `growthNewUrls`, `editorialNewUrls`, `moneyPageNewUrls`, `otherNewUrls`, `substantialRefreshes` en `otherGrowthActions`. Alleen `editorialNewUrls` en `moneyPageNewUrls` vullen de dagelijkse groeitarget. Zodra `newUrlDeficit` groter is dan nul, kiest cadence de hoogst scorende veilige kandidaat uit `allowedPublicationLanes`; bij een redactioneel tekort of een gevulde money-pagecap is `requiredPublicationLane=editorial` bindend. De oorspronkelijke toestandactie blijft als `companionAction` verplicht.
...
Kies van boven naar beneden de eerste unieke, veilige en uitvoerbare kans binnen `allowedPublicationLanes`. Wanneer `requiredPublicationLane=editorial`, worden money pages en commerciële landingspagina's voor die run overgeslagen:
...
De live cadence-check draait bewust niet als mergeblokker in CI. De dagelijkse automation behandelt exitcode `2` als uitvoeropdracht. Bij `newUrlRequired=true` is dat expliciet een nieuwe URL uit de gevalideerde backlog en de toegestane lane; de toestandactie blijft als begeleidende optimalisatie bestaan. Een reeds door een extra handmatige publicatie gevulde 7-daagse target voorkomt dubbel publiceren in een tweede run op dezelfde dag.
