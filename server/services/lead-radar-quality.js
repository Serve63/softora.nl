...
function createLeadRadarQuality(deps) {
  ...
  function classifySignal(input = {}) {
    ...
    const professionalRequest = /\b(?:ik\s+(?:zoek|ben\s+op\s*zoek\s+naar)|(?:wij|we)\s+(?:zoeken|zijn\s+op\s*zoek\s+naar)|(?:ben\s+ik|zijn\s+(?:wij|we))\s+op\s*zoek\s+naar)\s+(?:een\s+)?(?:(?:goede?|betaalbare?|ervaren|betrouwbare|creatieve|enthousiaste|zelfstandige|freelance|nederlandse|lokale|en)\s+){0,6}(?:(?:wordpress|shopify|webflow)[ -])?(?:webdesigner|webdeveloper|websitebouwer|webshopbouwer|softwareontwikkelaar|(?:app|software)[ -](?:ontwikkelaar|developer))\b/i.test(message);
