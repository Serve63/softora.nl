...
function isValidWebdesignImageDataUrl(value, normalizeString = defaultNormalizeString) {
...
function buildRowIdentityKey(row, normalizeString = defaultNormalizeString) {
  return [getRowCompany(row, normalizeString), getRowContact(row, normalizeString), getRowPhone(row, normalizeString)]
    .map((value) => normalizeString(value).toLowerCase())
    .join('|');
}

function buildCustomerPhotoDataKey(row, normalizeString = defaultNormalizeString) {
  return `softora_database_photo_data_v1_${normalizeString(getRowId(row, 0, normalizeString)).replace(/[^a-z0-9_-]+/gi, '_').slice(0, 80)}`;
}
...
function readChunkedCustomerPhoto(values, photoKey, chunkCount = 0, normalizeString = defaultNormalizeString) {
  ...
  return isValidWebdesignImageDataUrl(dataUrl, normalizeString) ? dataUrl : '';
}

function parseCustomerPhotoMap(raw, values = {}, rows = [], normalizeString = defaultNormalizeString) {
  let parsed = {};
  try {
    const candidate = JSON.parse(normalizeString(raw || '{}'));
    parsed = candidate && typeof candidate === 'object' && !Array.isArray(candidate) ? candidate : {};
  } catch (_) {
    parsed = {};
  }
  ...
  Object.keys(parsed).forEach((key) => {
    ...
    const chunked = readChunkedCustomerPhoto(
      stateValues,
      item.photoKey,
      Math.max(0, Math.min(80, Number(item.chunkCount || 0) || 0)),
      normalizeString
    );
    if (chunked) item.websitePhoto = chunked;
  });
  (Array.isArray(rows) ? rows : []).forEach((row, index) => {
    ...
    const chunked = readChunkedCustomerPhoto(
      stateValues,
      buildCustomerPhotoDataKey(row, normalizeString),
      0,
      normalizeString
    );
    if (!chunked) return;
    parsed[id] = {
      id,
      identityKey: buildRowIdentityKey(row, normalizeString),
      websitePhoto: chunked,
...
function normalizeInstantlyConfig(config = {}) {
  return {
    ...
    requireWebdesignAssets:
      config.requireWebdesignAssets === undefined
        ? true
        : readBool(config.requireWebdesignAssets, true),
...
function createInstantlyOutreachService(deps = {}) {
  ...
  async function loadCustomerPhotoMap(rows = []) {
    if (!config.requireWebdesignAssets) return {};
  ...
  function hasWebdesignAsset(row, id, index, photoMap) {
    if (!config.requireWebdesignAssets) return true;
    const inlinePhoto = row && (row.websitePhoto || row.photo || row.websiteImage);
    if (isValidWebdesignImageDataUrl(inlinePhoto, normalizeString)) return true;
    const photos = photoMap && typeof photoMap === 'object' ? photoMap : {};
    const direct = photos[id] || photos[getRowId(row, index, normalizeString)];
    if (isValidWebdesignImageDataUrl(direct && direct.websitePhoto, normalizeString)) return true;
    const identityKey = buildRowIdentityKey(row, normalizeString);
    return Object.keys(photos).some((key) => {
      const photo = photos[key];
      return (
        normalizeString(photo && photo.identityKey) === identityKey &&
        isValidWebdesignImageDataUrl(photo && photo.websitePhoto, normalizeString)
      );
    });
  }
  ...
  async function collectEligibleRows(rows, limit, photoMap = {}) {
    const selectedRows = [];
    const failed = [];
    ...
    for (let index = 0; index < rows.length && selectedRows.length < limit; index += 1) {
      ...
      if (!hasWebdesignAsset(row, id, index, photoMap)) {
        failed.push({
          id,
          bedrijf: company,
          email,
          error: `Geen webdesign/mockup gevonden voor ${company || email}.`,
        });
        continue;
      }
  ...
  async function syncInstantlyLeadsUnlocked(input = {}) {
    ...
    const photoMap = await loadCustomerPhotoMap(rows);
    const { selectedRows, failed } = await collectEligibleRows(rows, limit, photoMap);
