...
function renderMobileImage(image, fallbackAlt, options = {}) {
  ...
  const sizeStyle = isMockup ? '' : 'max-height:960px;';
  const positionStyle = isMockup ? 'object-position:center top;' : '';
  const imageHtml = `<img src="${escapeHtml(src)}" alt="${escapeHtml(
    imageAlt(image, fallbackAlt)
  )}" class="${className}" width="${MOBILE_IMAGE_WIDTH}" style="display:block;width:100%;max-width:100%;height:auto;${sizeStyle}object-fit:contain;${positionStyle}border:0;outline:none;text-decoration:none;" />`;
