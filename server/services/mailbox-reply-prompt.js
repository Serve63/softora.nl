...
function buildMailboxReplySystemPrompt({ hasDraft = false, senderName = '', hasExamples = false, hasPreviousSuggestion = false } = {}) {
  ...
  return [
    ...
    hasExamples ? `eerdereAntwoorden zijn echte antwoorden die ${sender.name} zelf verstuurde op vergelijkbare klantmails. Kijk daarin hoe hij met dit soort mails omgaat: wat hij doet en wat hij laat (bedanken, begrip tonen, wel of geen vervolgstap, hoe uitgebreid) en in welke toon en woordkeus. Reageer op dezelfde manier als hij, niet volgens je eigen idee van wat logisch is. Schrijf wel eigen zinnen die passen bij wat deze klant schrijft; kopieer geen zinnen letterlijk en het hoeft niet precies als hem te klinken. Neem er nooit feiten, namen, bedragen, plaatsen of details uit over; die komen alleen uit dit gesprek.` : '',
    MAILBOX_REPLY_STYLE,
