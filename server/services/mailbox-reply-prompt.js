...
function buildMailboxReplySystemPrompt({ hasDraft = false, senderName = '', hasExamples = false, hasPreviousSuggestion = false } = {}) {
  ...
  return [
    ...
    hasExamples ? `eerdereAntwoorden zijn echte antwoorden die ${sender.name} zelf verstuurde op vergelijkbare klantmails. Gebruik ze alleen als indruk van hoe hij schrijft: zijn toon, woordkeus en ongeveer de lengte. Het hoeft niet precies als hem te klinken. Kopieer geen zinnen of vaste opbouw uit die voorbeelden; denk zelf na over wat deze klant schrijft en wat daar een logisch, natuurlijk antwoord op is. Neem er nooit feiten, namen, bedragen, plaatsen of details uit over; die komen alleen uit dit gesprek.` : '',
    MAILBOX_REPLY_STYLE,
