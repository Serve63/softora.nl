...
if (!['postgres:', 'postgresql:'].includes(adminUrl.protocol) || !localHosts.has(adminUrl.hostname)) {
  throw new Error('Weiger tijdelijke UID-generation-testdatabase op een niet-lokale PostgreSQL-host.');
}

const databaseName = `softora_mailbox_uid_generation_test_${process.pid}_${crypto.randomBytes(5).toString('hex')}`;
const quotedDatabase = `"${databaseName.replaceAll('"', '""')}"`;
const testUrl = new URL(adminUrl);
testUrl.pathname = `/${databaseName}`;
const testFile = path.resolve(__dirname, 'mailbox-uid-generation-v2.postgres.test.js');
...
async function runTest() {
  ...
  created = true;
  process.stdout.write(`Tijdelijke UID-generation-testdatabase aangemaakt: ${databaseName}\n`);
...
runTest().catch((error) => {
  ...
  if (created) {
    ...
    process.stdout.write(`Tijdelijke UID-generation-testdatabase verwijderd: ${databaseName}\n`);
  }
