...
test('coldmail campaign attaches webdesign photo and device mockup inline and as attachments', async () => {
  ...
  assert.equal(result.sent, 1);
  assert.equal(sentMessages.length, 1);
  assert.match(sentMessages[0].html, /Bekijk je design op mobiel, tablet en desktop 👀👇/);
  assert.match(
    sentMessages[0].html,
  ...
  assert.ok(
    sentMessages[0].html.indexOf('<img src="cid:webdesign-prospect-1@softora"') <
      sentMessages[0].html.indexOf('Bekijk je design op mobiel, tablet en desktop 👀👇')
  );
  assert.ok(
    sentMessages[0].html.indexOf('Bekijk je design op mobiel, tablet en desktop 👀👇') <
...
test('coldmail campaign keeps the closing signature before webdesign photos', async () => {
  ...
  const captionIndex = html.indexOf('Bekijk je design op mobiel, tablet en desktop 👀👇');
