...
function assertWebdesignImagePairLayout(html, options = {}) {
  ...
  assert.match(value, /softora-webdesign-email-2026-07-15-v7/);
  ...
  assert.match(
    value,
    /class="softora-webdesign-email-body softora-coldmail-body" data-softora-template-version="softora-webdesign-email-2026-07-15-v7" style="font-family:Arial,sans-serif;font-size:16px;line-height:26px;color:#1a1a2e;width:100%;max-width:600px;min-width:0;box-sizing:border-box;overflow-wrap:anywhere;word-break:normal;/
  );
  assert.match(value, /<p style="margin:0 0 18px 0;font-family:Arial,sans-serif;font-size:16px;line-height:26px;color:#1a1a2e;max-width:100%;overflow-wrap:anywhere;word-break:normal;-webkit-text-size-adjust:100%;-ms-text-size-adjust:100%;text-size-adjust:100%;">/);
  assert.match(value, /\.softora-webdesign-email-body\{width:100%!important;max-width:600px!important;/);
  assert.match(value, /class="softora-webdesign-image-stack" style="display:block;[^\"]+max-width:600px/);
  assert.match(
    value,
    /class="softora-mockup-caption"[^>]*>Hieronder zie je een korte indruk van de eerste versie op verschillende schermen\.<\/p>/
  );
  assert.match(
    value,
    /class="softora-webdesign-image" width="600" style="display:block;width:100%;max-width:600px;height:auto;max-height:none;object-fit:contain;[^\"]*font-size:0;line-height:0;"/
  );
  assert.match(
    value,
    /class="softora-webdesign-image softora-webdesign-image--mockup" width="600" style="display:block;width:100%;max-width:600px;height:auto;max-height:none;object-fit:contain;object-position:center top;[^\"]*font-size:0;line-height:0;"/
  );

  const stackIndex = value.indexOf('class="softora-webdesign-image-stack"');
  const captionIndex = value.indexOf(COLDMAIL_MOCKUP_CAPTION_TEXT);
  assert.ok(stackIndex > 0, 'width-safe image stack is present');
  assert.equal(value.indexOf(COLDMAIL_MOCKUP_CAPTION_TEXT), value.lastIndexOf(COLDMAIL_MOCKUP_CAPTION_TEXT));
  assert.ok(captionIndex > stackIndex, 'caption appears inside the image stack');

  if (options.mobileMainAlt && options.mobileMockupAlt) {
    const mainIndex = value.indexOf(`alt="${options.mobileMainAlt}"`, stackIndex);
    const mockupIndex = value.indexOf(`alt="${options.mobileMockupAlt}"`, captionIndex);
    assert.ok(mainIndex > stackIndex, 'design image appears before the caption');
    assert.ok(captionIndex > mainIndex, 'caption appears after the design image');
    assert.ok(mockupIndex > captionIndex, 'mockup appears after the caption');
  }
...
function createService(overrides = {}) {
  ...
  const defaultMailboxSendProvenanceStore = {
    findByIdempotencyKey: async (key) => provenanceRows.get(key) || null,
    reserve: async (input) => {
      const existing = provenanceRows.get(input.idempotencyKey);
      if (existing) return { created: false, intent: existing };
      const intent = { ...input, status: 'prepared', dispatchState: 'reserved' };
      provenanceRows.set(input.idempotencyKey, intent);
      return { created: true, intent };
    },
    startDispatch: async (intentId) => {
      const intent = provenanceRows.get(intentId);
      if (intent) Object.assign(intent, { dispatchState: 'started' });
      return intent;
    },
    accept: async (intentId, values = {}) => {
      const intent = provenanceRows.get(intentId) || { intentId, idempotencyKey: intentId };
      Object.assign(intent, values, { status: 'accepted', dispatchState: 'finished' });
      provenanceRows.set(intentId, intent);
      return intent;
    },
    fail: async (intentId, error) => {
      const intent = provenanceRows.get(intentId) || { intentId, idempotencyKey: intentId };
      Object.assign(intent, { status: 'failed', dispatchState: 'finished', error });
      provenanceRows.set(intentId, intent);
      return intent;
    },
    markUnknown: async (intentId, error, values = {}) => {
      const intent = provenanceRows.get(intentId) || { intentId, idempotencyKey: intentId };
      Object.assign(intent, values, { status: 'unknown', dispatchState: 'started', error });
      provenanceRows.set(intentId, intent);
      return intent;
    },
  };
  ...
  const service = createColdmailCampaignService({
    ...
    dataOpsStore,
    mailboxSendProvenanceStore: overrides.mailboxSendProvenanceStore === null
      ? null
      : withMailboxPreDispatchProvenance(
        overrides.mailboxSendProvenanceStore || defaultMailboxSendProvenanceStore
      ),
...
test('coldmail campaign sends only eligible database rows and marks them as mailed', async () => {
  ...
  assert.match(sentMessages[0].html, /<p style="[^"]*font-size:16px;line-height:26px[^"]*">Goedemorgen Ruben,<\/p>/);
...
test('coldmail live stats count real sends from the guard and Softora/Gmail database signals', async () => {
  ...
  const { service } = createService({
    ...
    rows: [
      ...
      {
        id: 'bounce-provider-status',
        bedrijf: 'Provider Bounce BV',
        email: 'provider-bounce@example.test',
        status: 'geblokkeerd',
        databaseStatus: 'geblokkeerd',
        lastColdmailProvider: 'instantly',
        instantlyStatus: 'email_bounced',
        instantlyLastEventAt: '2026-04-22T09:15:00.000Z',
      },
      {
        id: 'bounce-history',
        bedrijf: 'Historie Bounce BV',
        email: 'historie-bounce@example.test',
        status: 'geblokkeerd',
        databaseStatus: 'geblokkeerd',
        hist: [
          { type: 'mail_bounce', label: 'Soft bounce via mailserver', date: '2026-04-21T09:15:00.000Z' },
        ],
      },
    ],
  });
...
test('coldmail campaign blocks Venvisual before SMTP when it would authenticate as Servé', async () => {
  const { service, sentMessages } = createService({
    mailboxAccountsRaw: JSON.stringify([
      {
        email: 'contact.venvisuals@gmail.com',
...
test('coldmail autopilot does not let a cooling lower-round sender block a ready sender', async () => {
  const senderEmails = ['serve@softora.nl', 'martijn@softora.nl'];
  const { service, sentMessages, getAutopilotState } = createService({
    rows: [
      {
        id: 'prospect-1',
    ...
    sendGuardState: {
      entries: [
        {
          at: '2026-06-22T05:10:00.000Z',
          senderEmail: 'serve@softora.nl',
          count: 1,
          personalCount: 0,
        },
        {
          at: '2026-06-22T06:59:00.000Z',
          senderEmail: 'serve@softora.nl',
          count: 1,
          personalCount: 0,
        },
    ...
    autopilotState: {
      ...
      lastStartedAt: '2026-06-22T07:54:00.000Z',
    },
...
test('coldmail autopilot does not hide an explicitly configured sender without smtp', async () => {
  const { service, sentMessages, getAutopilotState } = createService({
    ...
    autopilotState: {
      ...
      config: {
        count: 1,
        senderEmails: ['serve@softora.nl', 'martijnven123@gmail.com'],
        senderProfiles: {
          'serve@softora.nl': {
            subject: 'Korte vraag voor {{bedrijf}}',
            body: 'Goedemorgen {{naam}}, zou u openstaan voor een betere website?',
...
test('coldmail autopilot uses day quota instead of blocking on yesterday rolling sends', async () => {
  const martijnRollingEntries = Array.from({ length: 9 }, (_, index) => ({
    ...
    count: 1,
    personalCount: 0,
    recipientEmail: `old-${index}@example.test`,
    recipientDomain: `old-${index}-example-test`,
  }));
  const { service, sentMessages, getAutopilotState } = createService({
    rows: [
      {
    ...
    mailboxAccountsRaw: JSON.stringify([
      ...
      },
      {
        ...
        smtpHost: 'smtp.gmail.com',
        smtpUser: 'martijnven123@gmail.com',
        smtpPass: 'martijn-secret',
      },
    ]),
    ...
    sendGuardState: {
      entries: [
        {
          at: '2026-06-11T05:05:00.000Z',
          senderEmail: 'serve@softora.nl',
          count: 1,
          personalCount: 0,
          recipientEmail: 'already-today@example.test',
          recipientDomain: 'already-today-example-test',
        },
        ...martijnRollingEntries,
      ],
    },
    autopilotState: {
      enabled: true,
      ...
      lastStartedAt: '2026-06-11T06:50:00.000Z',
    },
...
test('coldmail autopilot uses the saved dashboard profile and attaches webdesign plus mockup', async () => {
  const { service, sentMessages, getAutopilotState } = createService({
    env: {
  ...
  assert.deepEqual(sentMessages[0].attachments.map((item) => item.filename), ['Webdesign.png', 'Mockup.png']);
  assert.ok(sentMessages[0].attachments.every((item) => item.contentDisposition === 'attachment'));
  assert.ok(sentMessages[0].attachments.every((item) => item.cid === undefined));
...
test('coldmail autopilot treats fris webdesign dashboard text as a real image-backed webdesign send', async () => {
  ...
  assert.match(
    sentMessages[0].html,
    /Lukt het niet om de bijlage te openen\? Dan kun je het webdesign ook via deze <a href="https:\/\/www\.softora\.nl\/webdesign\/rolsteiger-net\?cid=import-5-db-mpfntuzo-cifdr3&amp;sender=serve" target="_blank" rel="noopener noreferrer" style="color:#0a66c2;text-decoration:underline;font-weight:400;">link<\/a> bekijken 🎨/
  );
  assert.doesNotMatch(sentMessages[0].html, /<img\b/i);
  assert.doesNotMatch(sentMessages[0].html, /cid:/);
  assert.equal(sentMessages[0].attachments.length, 2);
...
test('coldmail campaign can use durable remote webdesign photo and device mockup URLs', async () => {
  ...
  assert.equal(
    ...
    'softora-webdesign-email-2026-07-15-v7'
  );
  ...
  assert.match(
    sentMessages[0].html,
    /softora-webdesign-image-stack[\s\S]*<img src="https:\/\/www\.softora\.nl\/coldmailing\/webdesign-foto\?t=[^"]+"/
  );
  assert.doesNotMatch(sentMessages[0].html, /cid:/);
...
test('coldmail autopilot uses two regular attachments and no inline images', async () => {
  const { service, sentMessages } = createService({
    env: {
      COLDMAIL_WEBDESIGN_IMAGE_DELIVERY: 'cid',
    },
...
test('coldmail campaign prefers stored design photo records over stale row mockup copies', async () => {
  ...
  assert.equal(sentMessages[0].attachments[1].filename, 'Mockup.png');
});
...
test('coldmail auto-reply blocks a mismatched Venvisual SMTP login before OpenAI and SMTP', async () => {
  const parsedInbound = {
    messageId: '<incoming-venvisual-mismatch@example.test>',
    subject: 'Re: Kleine vraag over jullie website',
    text: 'Hoi Martijn, kun je meer informatie sturen?',
    from: { value: [{ address: 'prospect@example.test', name: 'Prospect' }] },
    to: { value: [{ address: 'contact.venvisuals@gmail.com', name: 'Martijn van de Ven' }] },
    cc: { value: [] },
    references: '<sent-venvisual@softora>',
  };
  let openAiCalled = false;
  const { service, sentMessages } = createService({
    imapHost: 'imap.example.test',
    ...
    rows: [
      {
        id: 'prospect-venvisual-mismatch',
        bedrijf: 'Prospect BV',
        naam: 'Prospect',
        ...
        campaignType: 'webdesign',
        lastColdmailSentAt: '2026-04-24T10:00:00.000Z',
        lastColdmailSenderEmail: 'contact.venvisuals@gmail.com',
        mail: true,
      },
    ],
    createImapClient: () => ({
    ...
    fetchJsonWithTimeout: async () => {
      ...
      return {
        ...
        data: {
          model: 'gpt-5.5-pro',
          choices: [{ message: { content: 'Dit mag nooit verstuurd worden.' } }],
        },
      };
    },
  });
  ...
  assert.equal(openAiCalled, false);
  assert.equal(sentMessages.length, 0);
  assert.ok(result.errors.some((message) => /SMTP-login hoort niet bij/.test(message)));
});
...
test('coldmail campaign voltooit de Sent-kopie als provenance en herstelmarkering na SMTP uitvallen', async () => {
  ...
  const { service } = createService({
    rows: [{
      ...
      email: 'info@provenance-outage.example', website: 'https://provenance-outage.example',
      status: 'prospect', mail: true,
    }],
    outboundRecipientGuardStore: {
      findRecipientConflict: async () => null,
...
test('coldmail campaign releases the central reservation when SMTP fails before accept', async () => {
  ...
  const { service, sentMessages } = createService({
    ...
    sendMailError: Object.assign(new Error('535 Authentication failed'), { responseCode: 535 }),
  });
...
test('coldmail guard-responseverlies gebruikt de vooraf bekende reservation-ID en faalt zichtbaar bij releaseverlies', async (t) => {
  for (const releaseSucceeds of [true, false]) {
    await t.test(releaseSucceeds ? 'release bevestigd' : 'release niet bevestigd', async () => {
      const calls = [];
      let providerCalls = 0;
      let expectedReservationId = '';
      const reserveError = Object.assign(new Error('guard committeerde maar response ging verloren'), {
        code: 'ETIMEDOUT',
      });
      const { service } = createService({
        rows: [{
          id: `guard-response-loss-${releaseSucceeds}`, bedrijf: 'Guard Response BV',
          naam: 'Guard Response BV', email: `info@guard-response-${releaseSucceeds}.example`,
          website: 'https://guard-response.example', status: 'prospect', mail: true,
        }],
        outboundRecipientGuardStore: {
          findRecipientConflict: async () => null,
          reserveRecipients: async (_items, options) => {
            expectedReservationId = options.reservationId;
            calls.push(`reserve:${expectedReservationId}`);
            throw reserveError;
          },
          releaseReservation: async (reservationId) => {
            calls.push(`release:${reservationId}`);
            return { ok: releaseSucceeds };
          },
        },
        onSendMail: async () => { providerCalls += 1; },
      });
      ...
      await assert.rejects(() => service.sendColdmailCampaign({
        ...
        if (releaseSucceeds) return error.code === 'SMTP_SEND_FAILED';
        return error.code === 'COLDMAIL_SAFETY_PAUSED'
          && error.failedItems.some((item) => item.code === 'COLDMAIL_OUTBOUND_GUARD_RELEASE_FAILED');
      });

      assert.match(expectedReservationId, /^coldmail-send-/);
      assert.deepEqual(calls, [
        `reserve:${expectedReservationId}`,
        `release:${expectedReservationId}`,
      ]);
  ...
  for (const failingStep of ['reserve', 'start']) {
    await t.test(failingStep, async () => {
      ...
      if (failingStep === 'reserve') {
        assert.equal(calls.some((call) => call.startsWith('guard:reserve:')), false);
      }
...
test('remote webdesign sent copy contains responsive HTML and no image attachment parts', async () => {
  ...
  assert.match(String(parsedSentCopy.html || ''), /softora-webdesign-email-2026-07-15-v7/);
