...
test('coldmail campaign attaches webdesign photo and device mockup inline and as attachments', async () => {
  ...
  assert.match(
    sentMessages[0].html,
    /<p style="margin:16px 0 0 0;"><img src="cid:webdesign-prospect-1@softora"/
  );
  assert.ok(
    sentMessages[0].html.indexOf('Zo ziet het eruit op elk device 🤩') <
...
test('coldmail campaign places webdesign photos before the closing signature', async () => {
  ...
  assert.ok(imageIndex > mockupIndex);
  assert.ok(closingIndex > imageIndex);
