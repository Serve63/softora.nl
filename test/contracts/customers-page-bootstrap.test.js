...
test('customers page bootstrap reads stored customers through the repository seam first', async () => {
  ...
  const service = createCustomersPageBootstrapService({
    getUiStateValues: async (scope) => {
      ...
      return {
        values: {
          softora_customers_premium_v1: 'not-json',
        },
      };
    },
    customerRepository: {
  ...
  assert.equal(payload.customers[0].naam, 'Repo Klant');
  assert.equal(payload.customers[0].verantwoordelijk, 'Martijn');
  assert.equal(rawCustomerReads, 0);
});

test('customers page bootstrap keeps chunked legacy fallback when repository has no rows', async () => {
  const rawCustomers = JSON.stringify([
    {
      id: 'klant-chunked',
      naam: 'Chunked Klant',
      bedrijf: 'Chunked B.V.',
      telefoon: '+31 6 99 88 77 66',
      type: 'Website',
      website: 'chunked.nl',
      websiteBedrag: 450,
      status: 'Betaald',
    ...
    getUiStateValues: async (scope) => {
      ...
      return {
        values: {
          ...
          softora_customers_premium_v1_chunk_0: rawCustomers.slice(0, 25),
          softora_customers_premium_v1_chunk_1: rawCustomers.slice(25),
        },
      };
    },
    customerRepository: {
  ...
  assert.equal(payload.source, 'customers');
  assert.equal(payload.customers.length, 1);
  assert.equal(payload.customers[0].naam, 'Chunked Klant');
  assert.equal(payload.customers[0].websiteBedrag, 450);
});
