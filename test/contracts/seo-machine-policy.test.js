...
test('SEO machine policy requires one automation with a daily public growth output', () => {
  ...
  assert.match(policy, /zeven nieuwe groei-URL's per rollende 7 dagen[\s\S]*minimaal vijf `editorial`[\s\S]*maximaal twee `money_page`/i);
  assert.match(policy, /`performance_recovery` is geen publicatiestop[\s\S]*publiceert de machine dagelijks/i);
  assert.match(policy, /`newUrls`[\s\S]*`editorialNewUrls`[\s\S]*`moneyPageNewUrls`[\s\S]*`substantialRefreshes`[\s\S]*`otherGrowthActions`/i);
  assert.match(policy, /Alleen `editorialNewUrls` en `moneyPageNewUrls` vullen de dagelijkse groeitarget/i);
  ...
  assert.match(qualityGates, /Iedere succesvolle niet-P0-run stuurt op een nieuwe groei-URL/i);
