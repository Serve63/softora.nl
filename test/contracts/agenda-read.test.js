...
test('agenda read coordinator lets admin personeel accounts see the same private appointments', async () => {
  const appointments = [
    {
      id: 51,
      company: 'Martijn privé afspraak',
  ...
  const serveResult = await coordinator.listAppointments({
    limit: 10,
    viewer: {
      authenticated: true,
  ...
  const martijnResult = await coordinator.listAppointments({
    limit: 10,
    viewer: {
      authenticated: true,
  ...
  assert.equal(serveResult.appointments[0].privacyMasked, undefined);
  assert.equal(martijnResult.appointments[0].privacyMasked, undefined);
  assert.equal(serveResult.appointments[0].company, 'Martijn privé afspraak');
  assert.deepEqual(martijnResult.appointments, serveResult.appointments);
});
