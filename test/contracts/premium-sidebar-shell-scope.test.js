...
test('premium database consistency assets stay outside the static sidebar', () => {
  ...
  assert.ok(source.indexOf('assets/premium-database-mail-ready-menu.js?v=20260914-provider') > asideEnd);
...
test('premium mailbox behoudt alleen de vaste premium-sidebar bij responsive mailweergave', () => {
  ...
  assert.match(pageSource, /\.detail-mail-contact-item:has\(> \.detail-mail-contact-address\) \{\s*display:\s*block;\s*\}/);
  assert.match(pageSource, /assets\/premium-mailbox-message-presentation\.js\?v=20260921a/);
  ...
  assert.match(pageSource, /assets\/premium-mailbox-signature\.js\?v=20260910b/);
...
test('database loading repair keeps its premium shell and serves matching design eligibility assets', () => {
  ...
  assert.match(source, /premium-database-webdesign-action\.js\?v=20260915-manual-instantly/);
});
