...
test('premium instellingen gebruikt delegated actions zonder inline handlers', () => {
  ...
  assert.match(userManagementSource, /var isToto = label === 'Ruben zet toto';/);
  assert.match(userManagementSource, /var isLinkedModule = isWinning \|\| isDatabase \|\| isHealth \|\| isOmzetwerk \|\| isToto;/);
  assert.match(userManagementSource, /card\.setAttribute\('data-settings-extra-href', moduleHref\);/);
  assert.match(userManagementSource, /\? '\/premium-gezondheidsdossier'[\s\S]*\? '\/premium-omzetwerk'[\s\S]*: '#toto-forward-lab';/);
  ...
  assert.match(userManagementSource, /if \(isToto\) \{[\s\S]*window\.SoftoraToto\.open\(\)/);
  ...
  assert.match(source, /premium-toto\.css\?v=20260726a/);
  assert.match(source, /premium-ui-state-client\.js\?v=20260722b/);
  assert.match(source, /premium-toto-math\.js\?v=20260726a/);
  assert.match(source, /premium-toto\.js\?v=20260726a/);
