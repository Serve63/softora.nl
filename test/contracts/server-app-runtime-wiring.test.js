...
test('server app runtime wiring composes AI dashboard coordinators into feature routes', () => {
  ...
  assert.ok(capturedRouteOptions.premiumDatabaseCinematicJobsCoordinator);
  assert.equal(
    typeof capturedRouteOptions.premiumDatabaseCinematicJobsCoordinator.startJobResponse,
    'function'
  );
  assert.equal(
  ...
  assert.ok(capturedRouteOptions.publicConversionCoordinator);
  assert.equal(typeof capturedRouteOptions.publicConversionCoordinator.recordResponse, 'function');
