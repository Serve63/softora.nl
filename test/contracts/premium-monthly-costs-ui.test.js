...
test('premium maandelijkse kosten gebruikt dashboard-typografie en verbergt legacy kostenblokken', () => {
  ...
  assert.match(pageSource, /\.monthly-costs-stage\s*\{[\s\S]*min-height:\s*420px;[\s\S]*transition:\s*opacity \.16s ease;/);
  assert.match(pageSource, /\.monthly-costs-stage\.is-booting\s*\{[\s\S]*opacity:\s*0;[\s\S]*pointer-events:\s*none;/);
  ...
  assert.match(pageSource, /<div class="monthly-costs-stage is-booting" id="monthly-costs-stage" aria-busy="true">/);
...
test('premium maandelijkse kosten bewaart bewerkbare posten via supabase ui-state', () => {
  ...
  assert.match(pageSource, /function setMonthlyCostsStageBooting\(isBooting\) \{/);
