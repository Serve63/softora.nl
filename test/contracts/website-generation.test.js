...
test('website generation helpers build preview prompt, brief and filename from scan data', () => {
  ...
  assert.match(prompt, /volledig nieuwe ultra-premium redesign/i);
  assert.match(prompt, /veel sterkere nieuwe layout, compositie en art direction/i);
  assert.match(prompt, /bespoke, award-worthy elite website van top-agency niveau/i);
  assert.match(prompt, /editorial art direction, sterke typografische hiërarchie, asymmetrische grid/i);
  assert.match(prompt, /visual storytelling, royale ruimte, immersive depth, luxe details/i);
  assert.match(prompt, /iconische hero en een krachtige high-end brand experience/i);
  ...
  assert.match(prompt, /gedurfde maar logische compositie/i);
  assert.match(prompt, /sterke, realistische en premium websiteteksten/i);
  assert.match(prompt, /overtuigende headline, slimme subheadings, geloofwaardige waardeproposities/i);
  assert.match(prompt, /conversion-focused call-to-actions/i);
