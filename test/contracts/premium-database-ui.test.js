...
test('premium database page bootstraps customer rows before async sync runs', () => {
  ...
  assert.match(pageSource, /assets\/premium-database-photo-actions\.js\?v=20260428b/);
  assert.match(photoActionsSource, /class=\\"photo-generate/);
  assert.match(photoActionsSource, /data-generate-photo-id=\\"/);
  assert.match(photoActionsSource, /class=\\"photo-spinner\\"/);
  assert.match(photoActionsSource, /@keyframes softora-photo-spin/);
  assert.match(photoActionsSource, /\.webdesign-count-input:focus/);
  assert.match(photoActionsSource, /box-shadow:0 0 0 3px rgba\(155,35,85,\.22\)/);
  assert.match(photoActionsSource, /function collapseCountInputSelection\(input\)/);
  assert.match(photoActionsSource, /compactText\.indexOf\("webdesignsmaken"\)/);
  ...
  assert.match(photoActionsSource, /async function generateWebdesignPhotosWithInlineLoaders\(\)/);
  assert.match(photoActionsSource, /generateWebdesignPhotos = generateWebdesignPhotosWithInlineLoaders;/);
  assert.doesNotMatch(photoActionsSource, /setStatusMessage\("Webdesign maken/);
