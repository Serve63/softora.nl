...
test('premium database toont Supabase-hapering zonder data als leeg te presenteren', () => {
  ...
  assert.match(pageSource, /assets\/premium-database-webdesign-mockup\.js\?v=20260606b/);
  ...
  assert.match(webdesignMockupScriptSource, /const DEVICE_MOCKUP_VERSION = "v12";/);
  assert.match(webdesignMockupScriptSource, /const MOCKUP_BACKGROUND_SRC = "\/assets\/webdesign-preview-stage-bg\.jpg";/);
  assert.match(webdesignMockupScriptSource, /const MOCKUP_TEMPLATE_SRC = "\/assets\/webdesign-device-mockup-template-v12\.jpg";/);
  assert.match(webdesignMockupScriptSource, /const DEVICE_MOCKUP_SCREENS = \[/);
  assert.match(webdesignMockupScriptSource, /function loadMockupBackground\(\)/);
  assert.match(webdesignMockupScriptSource, /function loadMockupTemplate\(\)/);
  assert.match(webdesignMockupScriptSource, /function drawStageBackground\(context, backgroundImage\)/);
  assert.match(webdesignMockupScriptSource, /function drawWebsiteOnTemplateScreen\(context, image, screen\)/);
  assert.match(webdesignMockupScriptSource, /function clipScreenPolygon\(context, screen\)/);
