...
test('database boot draws the complete screen once after visible photos are ready', async () => {
  ...
  await databaseBoot.run({
    ...
    snapshotClient: { markCanonicalInventoryReady: () => true },
    loadMailReadySnapshot: async () => {},
    bootstrapCustomers: async (options) => {
      assert.equal(options.skipPhotoRestore, true);
      assert.equal(options.deferRender, true);
      state.klanten = [{ id: 'customer-1' }];
      return true;
    },
    getFilteredCustomers: () => state.klanten,
    getSortedCustomers: (customers) => customers,
    webdesignActionController: { preloadPhotoImages: async () => {
      assert.equal(state.photoRestorePending, true);
      assert.equal(events.includes('render'), false);
      events.push('preload');
    } },
    renderPage: () => {
      assert.equal(state.photoRestorePending, false);
      events.push('render');
    },
    databaseReadiness: { publish: async () => { events.push('ready'); } },
    releaseDatabaseBootShell: () => {},
    databasePendingJobsPromise: Promise.resolve(),
  ...
  assert.ok(events.indexOf('preload') < events.indexOf('render'));
...
test('database boot publishes persisted data without waiting for provider maintenance', async () => {
  ...
  const boot = databaseBoot.run({
    ...
    snapshotClient: { markCanonicalInventoryReady: () => true },
    loadMailReadySnapshot: async () => {},
    bootstrapCustomers: async () => { state.klanten = [{ id: 'one' }]; },
    getFilteredCustomers: () => state.klanten,
    getSortedCustomers: (customers) => customers,
    webdesignActionController: { preloadPhotoImages: async () => {} },
    renderPage: () => { events.push('render'); },
    databaseReadiness: { publish: async () => { events.push('ready'); } },
    releaseDatabaseBootShell: () => { events.push('release'); },
    databasePendingJobsPromise: Promise.resolve(),
    databaseImportController: {
      prepareAutoSync: () => Promise.resolve({}),
      startAutoSync: async () => ({ ok: true, configured: false }),
    },
...
function loadDatabaseSystemMailCountClient(options = {}) {
  ...
  const sandbox = { window: windowObject, fetch: options.fetch, setInterval: windowObject.setInterval, Date: options.Date || Date };
  vm.runInNewContext(source, sandbox);
...
function loadDatabaseMailReadySnapshotClient(options = {}) {
  const scriptPath = path.join(__dirname, '../../assets/premium-database-mail-ready-snapshot.js');
  const source = fs.readFileSync(scriptPath, 'utf8');
  const windowObject = {
...
function loadDatabaseWebdesignActionClient(options = {}) {
  ...
  const windowObject = {
    ...
    SoftoraDatabaseInstantlyStatus: require('../../assets/premium-database-instantly-status'),
    URL,
  };
...
test('premium database keeps bootstrap rows hidden until the canonical inventory is ready', () => {
  const pagePath = path.join(__dirname, '../../premium-database.html');
  const pageSource = fs.readFileSync(pagePath, 'utf8');
  const mailReadySnapshotSource = fs.readFileSync(path.join(__dirname, '../../assets/premium-database-mail-ready-snapshot.js'), 'utf8');
...
test('snapshot refresh keeps the last coherent found inventory until a complete replacement arrives', async () => {
  ...
  const state = {
    ...
    foundSnapshotCustomerIdSet: new Set(['found-1', 'found-2']),
  };
  ...
  const loaded = await client.load({
    ...
    fetchJsonWithTimeout: async () => ({
      ok: true,
      json: async () => ({ ok: true, total: 0, customers: [], availableTotal: 0, availableCustomers: [], foundTotal: 2, foundCustomerIds: ['incomplete'] }),
    }),
  });

  assert.equal(loaded, true);
  assert.equal(state.foundSnapshotLoaded, true);
  assert.equal(state.foundSnapshotTotal, 2);
...
test('premium database has a compact company search without the old result count card', () => {
  ...
  assert.match(
    pageSource,
    /function getVisibleTableCustomers\(customers\) \{ return customers \|\| \[\]; \}/
  );
  assert.match(
    pageSource,
...
test('premium database mail ROI calculator uses the live Softora mail count', async () => {
  ...
  assert.equal(requestedUrls[0], '/api/coldmailing/stats?includeRecipients=1');
...
test('premium database mail counter combines Softora with confirmed Instantly sends only', async () => {
  const nodes = {
    systemMailSentTodayCount: { textContent: '' },
    systemMailBouncesTodayCount: { textContent: '' },
    systemMailSentCount: { textContent: '', title: '' },
    mailRoiAppointmentsCount: { textContent: '' },
    mailRoiAppointmentRatio: { textContent: '' },
    mailRoiDealsCount: { textContent: '' },
    mailRoiRatio: { textContent: '' },
  };
  const client = loadDatabaseSystemMailCountClient({
    document: { hidden: false, getElementById: (id) => nodes[id] || null, querySelectorAll: () => [] },
    fetch: async () => ({ ok: true, json: async () => ({ ok: true, stats: { systemTotalSent: 100 } }) }),
    setInterval: () => 0,
  });
  const customers = [
    { email: 'sent@example.test', lastColdmailProvider: 'instantly', instantlyEmailSentAt: '2026-09-14T10:00:00Z' },
    { email: 'opened@example.test', lastColdmailProvider: 'instantly', instantlyStatus: 'opened' },
    { email: 'sent@example.test', lastColdmailProvider: 'instantly', instantlyStatus: 'reply_received' },
    { email: 'queued@example.test', lastColdmailProvider: 'instantly', instantlyStatus: 'queued' },
    { email: 'synced@example.test', lastColdmailProvider: 'instantly', instantlyStatus: 'synced' },
    { email: 'manual@example.test', lastColdmailProvider: 'instantly', instantlyStatus: 'sent', instantlyManualSentClassification: 'user-requested-unverified' },
  ];

  assert.equal(client.getInstantlySystemMailSentCount(customers), 3);
  assert.equal(client.getInstantlySystemMailSentTodayCount(customers, {
    now: () => new Date('2026-09-14T18:00:00.000+02:00'),
  }), 1);
  client.render(customers, { dataLoading: false });
  await client.refreshTodaySentCount();

  assert.equal(nodes.systemMailSentCount.textContent, '103');
  assert.equal(nodes.systemMailSentCount.title, 'Softora en Instantly samen; handmatig als verzonden gemarkeerde records tellen mee.');
});
...
test('premium database mail ROI calculator toont vandaag verstuurd en uitsluitend harde bounces', async () => {
      ...
      return {
        ...
        json: async () => ({
          ...
          stats: {
            sentToday: 31,
            systemSentToday: 31,
            instantlySentToday: 5,
            instantlyStatsReliable: true,
            bounces: 9,
            bouncesToday: 4,
            bounceStatsReliable: true, bounceStatsModel: "complete-mailbox-recipient-v2",
  ...
  assert.equal(nodes.systemMailSentTodayCount.textContent, '31 - 5 - 36');
  ...
  const nodes = {
    softoraCustomersBootstrap: {
      textContent: JSON.stringify({
        mailStats: { bounceStatsReliable: true, bounceStatsModel: "complete-mailbox-recipient-v2", sentToday: 0, bounces: 59, hardBounces: 27, totalSent: 2000 },
      }),
    },
    systemMailSentTodayCount: { textContent: '' },
    systemMailBouncesTodayCount: { textContent: '' },
      ...
      return {
        ...
        json: async () => ({
          ...
          stats: {
            ...
            bounceStatsReliable: requestCount === 1, bounceStatsModel: "complete-mailbox-recipient-v2",
...
test('premium database webdesign asset state keeps mail-ready and photo-target definitions separated', () => {
  ...
  const snapshotReady = assetStateClient.buildWebdesignAssetState({
    ...base,
    hasPhoto: true,
    hasMockup: true,
  ...
  assert.equal(snapshotReady.hasPhoto, true);
  assert.equal(snapshotReady.hasMockup, true);
  assert.equal(snapshotReady.isMailReady, true);
...
test('mail-ready snapshot client loads compact rows and the guarded scraper inventory before full database restore', async () => {
  ...
  const loaded = await client.load({
    useSnapshotArchive: false,
    databaseHadBootstrapCustomers: false,
    state,
    normalizeCustomer: (raw, fallbackId) => ({ id: raw.id || fallbackId, bedrijf: raw.bedrijf, email: raw.email }),
    applyCustomerList: (customers, forceRender) => applied.push({ customers, forceRender }),
    fetchJsonWithTimeout: async (...args) => {
      requests.push(args);
      return { ok: true, json: async () => ({ ok: true, total: 1, customers: [{ id: 'customer-ready', bedrijf: 'Demo BV', email: 'info@demo.nl', hasPhoto: true, hasMockup: true, mailReady: true }], availableTotal: 1, availableCustomers: [{ id: 'customer-available', bedrijf: 'Nieuw BV', email: 'info@nieuw.nl', hasPhoto: false, hasMockup: false, availableSnapshot: true }], foundTotal: 2, foundCustomerIds: ['customer-ready', 'customer-found'] }) };
    },
  });
  ...
  assert.equal(state.availableSnapshotLoaded, true);
  assert.equal(state.availableSnapshotTotal, 1);
  assert.equal(state.availableSnapshotCustomers.length, 1);
  assert.equal(state.foundSnapshotLoaded, true);
  assert.equal(state.foundSnapshotTotal, 2);
  assert.deepEqual(Array.from(state.foundSnapshotCustomerIdSet), ['customer-ready', 'customer-found']);
  assert.equal(client.isSnapshotFoundCustomer({ id: 'customer-found' }, state.foundSnapshotCustomerIdSet), true);
  assert.equal(client.isSnapshotFoundCustomer({ id: 'already-mailed' }, state.foundSnapshotCustomerIdSet), false);
  assert.equal(client.isFoundSnapshotCategoryCoherent(2, ['customer-ready', 'customer-found']), true);
  assert.equal(client.isFoundSnapshotCategoryCoherent(2, ['customer-ready']), false);
  assert.equal(state.dataUnavailable, false);
  assert.equal(applied.length, 1);
  assert.equal(applied[0].forceRender, false);
  assert.equal(applied[0].customers[0].websitePhotoAssetReady, true);
  ...
  assert.equal(client.isSnapshotAvailableCustomer(applied[0].customers[1]), true);
  ...
  const merged = client.mergeAssetFlags([
    ...
    { id: 'customer-available', bedrijf: 'Nieuw BV', email: 'info@nieuw.nl', website: 'https://nieuw.nl' },
    { id: 'customer-other', bedrijf: 'Anders BV', email: 'info@anders.nl', website: 'https://anders.nl' },
  ], state.mailReadySnapshotCustomers, state.availableSnapshotCustomers);
  assert.equal(merged[0].websitePhotoAssetReady, true);
  assert.equal(merged[0].websiteMockupAssetReady, true);
  assert.equal(client.isSnapshotMailReadyCustomer(merged[0]), true);
  assert.equal(client.isSnapshotAvailableCustomer(merged[1]), true);
...
test('mail-ready snapshot client publishes the complete archive after one request', async () => {
  const client = loadDatabaseMailReadySnapshotClient({ console: { warn() {} } });
  ...
  const requests = [];
  const state = { klanten: [], mailReadySnapshotLoaded: false, availableSnapshotLoaded: false,
    foundSnapshotLoaded: false, foundSnapshotCustomerIdSet: new Set() };
  const loaded = await client.load({
    state,
    normalizeCustomer: (raw) => ({ ...raw }),
    applyCustomerList: () => {},
    fetchJsonWithTimeout: async (url) => {
      ...
      return { ok: true, json: async () => ({
        ok: true, generatedAt: '2026-09-23T00:00:00.000Z', snapshotVersion: 'sha256:archive',
        total: 1, customers: [{ id: 'ready-1', mailReady: true }],
        availableTotal: availableCustomers.length, availableCustomers,
        instantlyReadyTotal: 0, instantlyReadyCustomers: [], foundTotal: 1, foundCustomerIds: ['ready-1'],
      }) };
    },
  });
  assert.equal(loaded, true);
  assert.deepEqual(requests, ['/api/premium-database/mail-ready-snapshot/archive']);
  assert.equal(state.availableSnapshotTotal, 5001);
  assert.equal(state.availableSnapshotCustomers.length, 5001);
  assert.equal(state.mailReadySnapshotPending, false);
});
...
test('Instantly-ready designs remain visible in their own canonical category after a refresh', async () => {
  ...
  const config = {
    state,
    normalizeCustomer: (raw) => ({ ...raw }),
    applyCustomerList: (customers, _forceRender, alreadyCanonical) => { assert.equal(alreadyCanonical, false); rows = client.reconcileCustomerList(state, customers); state.klanten = rows; },
...
test('snapshot publication hands an already merged remote list to the table once', async () => {
  const client = loadDatabaseMailReadySnapshotClient();
  const state = { klanten: [{ id: 'ready-1', email: 'ready@example.test', bedrijf: 'Updated name' }], mailReadySnapshotLoaded: false, availableSnapshotLoaded: false, foundSnapshotLoaded: false };
  let published;
  const loaded = await client.load({
    state,
    useSnapshotArchive: false,
    normalizeCustomer: (row) => ({ ...row }),
    applyCustomerList: (rows, _forceRender, alreadyCanonical) => { published = { rows, alreadyCanonical }; state.klanten = rows; },
    fetchJsonWithTimeout: async () => ({ ok: true, json: async () => ({
      ok: true, total: 1, customers: [{ id: 'ready-1', email: 'ready@example.test', mailReady: true, mailReadySnapshot: true }],
      availableTotal: 0, availableCustomers: [], instantlyReadyTotal: 0, instantlyReadyCustomers: [],
      foundTotal: 1, foundCustomerIds: ['ready-1'], generatedAt: '2026-09-23T00:00:00.000Z',
    }) }),
  });
  assert.equal(loaded, true);
  assert.equal(published.alreadyCanonical, true);
  assert.equal(published.rows.length, 1);
  assert.equal(published.rows[0].bedrijf, 'Updated name');
  assert.equal(client.isSnapshotMailReadyCustomer(published.rows[0]), true);
});

test('Instantly status separates designs to upload, current campaign queue, failed reservations and sends', () => {
  ...
  assert.equal(status.isReadyForUpload(design, normalize), true);
  ...
  assert.equal(status.isConfirmedSent(sent), true);
});
    ...
    fetchJsonWithTimeout: async (url) => {
      ...
      return {
        ...
        json: async () => ({
          ...
          snapshotVersion: 'sha256:stable-content',
          total: 0,
          customers: [],
          availableTotal: availableCustomers.length,
...
test('premium database applies remote customers once after guard and photo enrichment complete', () => {
  ...
  assert.match(pageSource, /const customersWithPhotos = mergeCustomersWithPhotos\(enrichedCustomers, photoMap, state\.klanten, true\);/);
  ...
  assert.match(pageSource, /state\.remoteCustomersLoaded = true;[\s\S]*applyCustomerList\(sortedCustomers, false, true, deferRender\);/);
...
test('premium database replaces a compatibility bootstrap with the complete canonical customer list', () => {
  ...
  assert.match(pageSource, /async function bootstrapCustomers\(bootstrapOptions\) \{ const skipPhotoRestore = Boolean\(bootstrapOptions && bootstrapOptions\.skipPhotoRestore\), deferRender = Boolean\(bootstrapOptions && bootstrapOptions\.deferRender\);/);
  assert.match(pageSource, /let photoMap = \{\}; if \(!skipPhotoRestore && !\(state\.mailReadySnapshotLoaded && state\.availableSnapshotLoaded\)\)/);
  assert.match(databaseBootSource, /if \(databaseHadBootstrapCustomers && state\.klanten\.length && !databaseHasFastSnapshotBootstrap\) \{ const canonicalCustomersPromise = bootstrapCustomers\(\{ skipPhotoRestore: true, deferRender: true \}\); await mailReadySnapshotPromise; await canonicalCustomersPromise; if \(!\(state\.canonicalInventoryReady && state\.mailReadySnapshotLoaded && state\.availableSnapshotLoaded\)\) try \{ const photoMap = await loadCustomerPhotoMap/);
  assert.match(databaseBootSource, /await canonicalCustomersPromise; if \(!\(state\.canonicalInventoryReady && state\.mailReadySnapshotLoaded && state\.availableSnapshotLoaded\)\) try \{ const photoMap = await loadCustomerPhotoMap\(state\.klanten/);
});
...
test('premium database keeps the server snapshot authoritative for available rows', () => {
  ...
  assert.match(pageSource, /const status = normalizeDatabaseStatus\(customer\.status \|\| customer\.databaseStatus, customer\);[\s\S]*\["gemaild", "interesse", "afspraak", "klant", "afgehaakt", "geblokkeerd", "buiten"\]/);
...
test('premium database toont Supabase-hapering zonder data als leeg te presenteren', () => {
  ...
  const snapshotPath = path.join(__dirname, '../../assets/premium-database-mail-ready-snapshot.js');
  const pageSource = fs.readFileSync(pagePath, 'utf8');
  ...
  assert.match(pageSource, /state\.dataUnavailable = !state\.canonicalInventoryReady;/);
  assert.match(pageSource, /window\.SoftoraPremiumDatabaseCustomers\.load\(\{ previousSnapshotVersion:/);
  assert.match(pageSource, /catch \(error\) \{[\s\S]*console\.warn\("Klanten laden via Supabase tijdelijk overgeslagen:", error\);[\s\S]*state\.dataUnavailable = !state\.canonicalInventoryReady;/);
  ...
  assert.match(pageSource, /assets\/premium-database-mail-ready-snapshot\.js\?v=20260924-readmodel/);
  assert.match(pageSource, /assets\/premium-database-current-campaign-media\.js\?v=20260924-targeted-visibility/);
  ...
  assert.match(snapshotSource, /state\.foundSnapshotCustomerIdSet = new Set/);
  ...
  assert.match(pageSource, /if \(\(state\.dataLoading \|\| state\.dataUnavailable\) && !state\.klanten\.length\) \{ setDatabaseTableBodyHtml\(""\);/);
  assert.match(pageSource, /state\.dataUnavailable = !state\.canonicalInventoryReady;/);
  assert.match(pageSource, /state\.dataLoading = true;[\s\S]*state\.dataUnavailable = false;[\s\S]*renderPage\(\);/);
  ...
  assert.match(pageSource, /const canonicalCustomers = window\.SoftoraDatabaseMailReadySnapshot\.mergeWithCanonicalSnapshots\(customersWithPhotos, state\.mailReadySnapshotCustomers, state\.availableSnapshotCustomers, state\.instantlyReadySnapshotCustomers\);/);
  assert.match(pageSource, /const sortedCustomers = getSortedCustomers\(outreachAutomation\.customers\);[\s\S]*applyCustomerList\(sortedCustomers, false, true, deferRender\);/);
  ...
  assert.match(pageSource, /if \(!state\.canonicalInventoryReady\) renderPage\(\);[\s\S]*setStatusMessage\(""\);/);
  ...
  assert.match(pageSource, /showOutreachActionColumn = state\.activeStatus === "benaderd" \|\| state\.activeStatus === "instantly-queued" \|\| state\.activeStatus === "instantly" \|\| state\.activeStatus === "verstuurd", showPhotoColumn = !showOutreachActionColumn/);
  ...
  assert.match(pageSource, /function getEmptyTableMessage\(\) \{[\s\S]*Geen mailklare bedrijven voor Softora\.[\s\S]*Geen mailklare bedrijven voor Instantly\.[\s\S]*Geen leads klaargezet in de actuele Instantly-campagnes\.[\s\S]*Geen beschikbare bedrijven\.[\s\S]*Geen bedrijven in deze filter\./);
  assert.match(pageSource, /const mailReadyPending = isMailReadyCalculationPending\(\), baseFiltered = getSortedCustomers\(getFilteredCustomers\(\)\), visibleCustomers = getVisibleTableCustomers\(baseFiltered\), blockForMailReadyPending = mailReadyPending && showPhotoColumn && !visibleCustomers\.length;/);
  ...
  assert.match(photoBatchScriptSource, /className = "photo-generate-charge-label";/);
  assert.match(photoBatchScriptSource, /function updateChargeLabelPositions\(\)/);
  assert.match(photoBatchScriptSource, /querySelectorAll\("\.photo-generate-charge-label"\)/);
  assert.match(photoBatchScriptSource, /root\.document\.body\.appendChild\(label\)/);
  ...
  assert.match(photoBatchScriptSource, /function formatGenerationCost\(generation\)/);
  assert.match(webdesignActionScriptSource, /createCostReporter/);
  assert.match(webdesignActionScriptSource, /costReporter\.report\(job\)/);
  assert.match(webdesignActionScriptSource, /costReporter\.show\(variant\);/);
  ...
  assert.match(webdesignActionScriptSource, /!hasPhotoSource \|\| isInlinePhotoSource\(photo\) \|\| loadedPhotoKeys\.has\(photoLoadKey\)/);
  assert.match(webdesignActionScriptSource, /!hasMockupSource \|\| isInlinePhotoSource\(mockup\) \|\| loadedPhotoKeys\.has\(mockupLoadKey\)/);
  ...
  assert.match(pageSource, /const customersWithPhotos = mergeCustomersWithPhotos\(enrichedCustomers, photoMap, state\.klanten, true\)/);
  assert.doesNotMatch(pageSource, /customersWithFallbackMedia/);
  ...
  assert.match(filterGroupsCssSource, /\.status-filter\s*\{[^}]*background: #fff;[^}]*border: 1px solid #e8e6e1;[^}]*border-radius: 10px;[^}]*padding: 7px 5px;/);
  ...
  assert.match(filterGroupsCssSource, /\.filter-metrics\s*\{[\s\S]*margin-left: 0;[\s\S]*gap: 0;/);
  ...
  const mailRoiNoteCssBlock = filterGroupsCssSource.match(/\.mail-roi-note\s*\{[\s\S]*?\}/)[0];
  assert.match(mailRoiNoteCssBlock, /grid-area: note;/);
  assert.match(mailRoiNoteCssBlock, /justify-content: flex-start;/);
  assert.match(mailRoiNoteCssBlock, /padding: 0;/);
  assert.match(mailRoiNoteCssBlock, /font-size: 10px;/);
  assert.match(mailRoiNoteCssBlock, /font-weight: 600;/);
  assert.match(mailRoiNoteCssBlock, /text-align: left;/);
  assert.match(filterGroupsCssSource, /"autopilot today bounces sent appointments appointment-ratio deals ratio"/);
  assert.match(filterGroupsCssSource, /@media \(max-width: 860px\)[\s\S]*"bounces sent"[\s\S]*"appointments appointment-ratio"[\s\S]*"deals ratio"/);
  assert.match(filterGroupsCssSource, /@media \(max-width: 860px\) \{[\s\S]*\.mail-roi-note\s*\{[\s\S]*padding-left: 0;/);
  assert.match(filterGroupsCssSource, /\.mail-roi-card\s*\{[\s\S]*min-width: 0;[\s\S]*border: 1px solid #e0ddd8;/);
  ...
  assert.match(systemMailCountScriptSource, /systemMailCombinedSentTodayCount/);
  assert.match(systemMailCountScriptSource, /Paars Softora; blauw Instantly; zwart samen\./);
  ...
  assert.match(filterGroupsCssSource, /\.mail-roi-card--today \.mail-roi-label,\s*\.mail-roi-card--bounces \.mail-roi-label\s*\{[\s\S]*color: var\(--crimson\);/);
  ...
  assert.match(webdesignBulkScriptSource, /title=\\"Annuleren\\"/);
  assert.match(webdesignBulkScriptSource, /confirmText: "Annuleren"/);
  assert.doesNotMatch(webdesignBulkScriptSource, /Rest annuleren/);
  ...
  assert.match(webdesignBulkScriptSource, /buildJobPayload\(target, "", "v2-visual-dna"\)/);
  ...
  assert.match(databaseBootSource, /if \(databaseHadBootstrapCustomers && state\.klanten\.length && !databaseHasFastSnapshotBootstrap\) \{/);
  assert.match(databaseBootSource, /const photoMap = await loadCustomerPhotoMap\(state\.klanten, \{ force: true, failOnError: true, requireStateKey: true, failOnIncomplete: true \}\);/);
  assert.match(databaseBootSource, /loadCustomerPhotoMap\(state\.klanten, \{ force: true, failOnError: true, requireStateKey: true, failOnIncomplete: true \}\)/);
  assert.match(pageSource, /photoMap = await loadCustomerPhotoMap\(enrichedCustomers, \{ force: true, failOnError: true, requireStateKey: true, failOnIncomplete: true \}\);/);
  ...
  assert.match(databaseBootSource, /state\.photoRestoreFailed = true; applyCustomerList\(snapshotClient\.mergeAssetFlags\(state\.klanten, state\.mailReadySnapshotCustomers, state\.availableSnapshotCustomers\), false, false, true\); console\.warn\("Databasefoto's laden voor boot tijdelijk overgeslagen:", error\);/);
  ...
  assert.match(databaseBootSource, /applyCustomerList\(snapshotClient\.mergeAssetFlags\(mergeCustomersWithPhotos\(state\.klanten, photoMap, state\.klanten\), state\.mailReadySnapshotCustomers, state\.availableSnapshotCustomers\), false, false, true\);/);
  assert.match(databaseBootSource, /else \{ const canonicalCustomersPromise = bootstrapCustomers\(\{ skipPhotoRestore: true, deferRender: true \}\); await mailReadySnapshotPromise; await canonicalCustomersPromise; \}/);
  assert.match(databaseBootSource, /await webdesignActionController\.preloadPhotoImages\(getSortedCustomers\(getFilteredCustomers\(\)\), 16, 1200\);/);
  ...
  assert.doesNotMatch(databaseBootSource, /releaseDatabaseBootShell\(\); void webdesignActionController\.preloadPhotoImages/);
  assert.match(pageSource, /const databasePendingJobsPromise = webdesignActionController\.resumePendingJobs\(\);/);
  assert.match(databaseBootSource, /state\.pendingJobsRestored = await pendingJobsOutcome/);
  ...
  assert.match(databaseBootSource, /await databaseImportController\.startAutoSync\(autoSyncConfigPromise\);/);
  ...
  assert.match(importScriptSource, /function startAutoSync\(preparedConfig\)/);
  assert.match(pageSource, /const CUSTOMER_DB_SYNC_SCOPE = "premium_customers_database_sync"/);
  assert.match(pageSource, /syncScope: CUSTOMER_DB_SYNC_SCOPE/);
  assert.doesNotMatch(pageSource, /syncScope: CUSTOMER_DB_SCOPE/);
...
test('premium database webdesign action fails closed when the picker does not return V2', async () => {
  ...
  const webdesignActionClient = loadDatabaseWebdesignActionClient({
    ...
    SoftoraDatabaseWebdesignVariantPicker: { choose: async () => null },
    fetch: async () => {
      fetchCalls += 1;
  ...
  const controller = webdesignActionClient.createController({
    state: {
      klanten: [{
        ...
        bedrijf: 'MagniVita voeding en vitaliteit',
        website: 'https://www.magnivita.nl/',
        dom: 'magnivita.nl',
        websitePhoto: '',
      }],
    },
    ...
    shouldShowWebsitePhoto: () => true,
    isValidWebsitePhotoDataUrl: () => false,
    resolveCustomerWebsiteUrl: () => 'https://www.magnivita.nl/',
    isWebdesignPhotoEligible: () => true,
    openWebsitePhotoPreview() {},
    setStatusMessage() {},
    renderPage() {},
...
test('premium database webdesign action fails closed when the V2 picker is unavailable', async () => {
  ...
  const messages = [];
  const webdesignActionClient = loadDatabaseWebdesignActionClient({
    SoftoraDatabaseWebdesignVariantPicker: null,
    fetch: async () => {
      fetchCalls += 1;
      throw new Error('fetch should not be called');
    },
  ...
  const controller = webdesignActionClient.createController({
    state: { klanten: [{ id: 'customer-no-picker', bedrijf: 'Bliv Makelaardij' }] },
    escapeHtml: (value) => String(value),
    shouldShowWebsitePhoto: () => true,
    isValidWebsitePhotoDataUrl: () => false,
    resolveCustomerWebsiteUrl: () => 'https://www.bliv.nl/',
  ...
  const result = await controller.generateForCustomer('customer-no-picker');
...
/<div class="status-filter-group status-filter-group--shared" aria-label="Mailsysteem opties">[\s\S]*data-s="beschikbaar"[^>]*>Beschikbaar<\/button>[\s\S]*id="mailReadyMenuButton"[^>]*>Mailklaar<\/summary>[\s\S]*data-mail-ready-status="benaderbaar">[\s\S]*>Softora<\/span>[\s\S]*id="mailReadySoftoraCount"[\s\S]*data-mail-ready-status="instantly-ready">[\s\S]*>Instantly<\/span>[\s\S]*id="mailReadyInstantlyCount"[\s\S]*<div class="status-filter-group status-filter-group--sent" aria-label="Verstuurd per kanaal">[\s\S]*data-s="verstuurd"[^>]*>Softora<\/button>[\s\S]*data-s="instantly"[^>]*>Instantly<\/button>/
  ...
  assert.match(pageSource, /<button class="sf-btn" data-s="instantly" type="button">Instantly<\/button>/);
...
test('Instantly separates waiting leads from confirmed deliveries', () => {
  ...
  assert.deepEqual(contacts.filter(item => controller.matchesStatusFilter(item, 'instantly-ready')).map(item => item.id), ['transferred', 'current-queued']);
  ...
  assert.match(controller.renderMeta(contacts[11], true), /Klaargezet voor Instantly/);
...
test('Instantly mail-ready menu and list mirror both current unsent campaigns and transition on confirmed send', () => {
  ...
  assert.deepEqual(readyIds(), ['serve', 'martijn', 'design-only']);
  ...
  assert.deepEqual(readyIds(), ['martijn', 'design-only']);
  assert.deepEqual(menuIds(), readyIds());
  ...
  assert.deepEqual(readyIds(), ['design-only']);
  assert.deepEqual(menuIds(), ['design-only']);
...
test('bounce display rejects legacy bootstrap and stale responses but accepts an authoritative correction', async () => {
  const node = { textContent: '' };
  const nodes = { systemMailBouncesTodayCount: node, systemMailSentCount: { textContent: '' }, softoraCustomersBootstrap: { textContent: JSON.stringify({ mailStats: { hardBounces: 40 } }) } };
  let stats = { bounceStatsReliable: true, bounceStatsModel: 'complete-mailbox-recipient-v2', bounceStatsUpdatedAt: '2026-09-09T10:00:00Z', bounceTypes: { hard: 42 } };
  const client = loadDatabaseSystemMailCountClient({ document: { hidden: false, getElementById: id => nodes[id], querySelectorAll: () => [] }, setInterval: () => 0, fetch: async () => ({ ok: true, json: async () => ({ ok: true, stats }) }) });
  client.render([]);
  assert.equal(node.textContent, '--');
  await client.refreshTodaySentCount(); assert.equal(node.textContent, '42');
  stats = { ...stats, bounceTypes: { hard: 38 }, bounceStatsReliable: false };
  await client.refreshTodaySentCount(); assert.equal(node.textContent, '42');
  stats = { ...stats, bounceStatsReliable: true, bounceStatsUpdatedAt: '2026-09-09T09:00:00Z' };
  await client.refreshTodaySentCount(); assert.equal(node.textContent, '42');
  stats = { ...stats, bounceTypes: { hard: 41 }, bounceStatsUpdatedAt: '2026-09-09T11:00:00Z' };
  await client.refreshTodaySentCount(); assert.equal(node.textContent, '41');
});

test('appointment count persists with customers and computes one appointment per sent mails after reload', async () => {
  const stored = { premium_database_mail_roi_v1: JSON.stringify({ dealCount: 4 }), premium_database_mail_appointments_v1: JSON.stringify({ appointmentCount: 7 }) };
  function setup() {
    const handlers = {};
    ...
    const buttons = [-1, 1].map(delta => ({ getAttribute: key => key === 'data-mail-roi-metric' ? 'appointments' : String(delta), addEventListener: (_, handler) => { handlers[delta] = handler; } }));
    const client = loadDatabaseSystemMailCountClient({ document: { getElementById: id => nodes[id], querySelectorAll: () => buttons }, setInterval: () => 0,
      SoftoraUiStateClient: { get: async () => ({ values: { ...stored } }), set: async (_, value) => { Object.assign(stored, value.patch); return { ok: true }; } },
      fetch: async () => ({ ok: true, json: async () => ({ ok: true, stats: { systemTotalSent: 3582 } }) }),
    });
    return { client, handlers, nodes };
  }
  const first = setup(); first.client.render([]); await first.client.loadPersistedDealCount(); await first.client.refreshTodaySentCount();
  assert.equal(first.nodes.mailRoiAppointmentsCount.textContent, '7');
  first.handlers[1]();
  assert.equal(JSON.parse(stored.premium_database_mail_appointments_v1).appointmentCount, 8); assert.equal(JSON.parse(stored.premium_database_mail_roi_v1).dealCount, 4);
  assert.equal(first.nodes.mailRoiAppointmentRatio.textContent, '1 op 448');
  const second = setup(); second.client.render([]); await second.client.loadPersistedDealCount(); await second.client.refreshTodaySentCount();
  assert.equal(second.nodes.mailRoiAppointmentsCount.textContent, '8');
  for (let i = 0; i < 9; i += 1) second.handlers[-1]();
  assert.equal(second.nodes.mailRoiAppointmentsCount.textContent, '0');
  assert.equal(second.nodes.mailRoiAppointmentRatio.textContent, '—');
});

test('appointment reload refreshes a primed page snapshot after a confirmed decrement', async () => {
...
test('Mailsysteem only reuses a successful recent stats read', async () => {
  let nowMs = 100000;
  let failNext = false;
  ...
  const client = loadDatabaseSystemMailCountClient({
    ...
    fetch: async () => failNext
      ? { ok: false, json: async () => ({ ok: false }) }
      : { ok: true, json: async () => ({ ok: true, stats: { systemTotalSent: 10 } }) },
  });
  assert.equal(client.getMetricReadiness().statsFresh, false);
  await client.refreshTodaySentCount();
  assert.equal(client.getMetricReadiness().statsFresh, true);
  nowMs += 15001;
  assert.equal(client.getMetricReadiness().statsFresh, false);
  failNext = true;
  await client.refreshTodaySentCount();
  assert.equal(client.getMetricReadiness().stats, false);
  assert.equal(client.getMetricReadiness().statsFresh, false);
});
...
test('database places eight compact metrics next to filters and search on its own row', () => {
  ...
  const filters = pageSource.indexOf('<div class="filter-bar">');
  assert.ok(pageSource.indexOf('aria-label="Database statistieken"') > filters);
  const order = ['systemMailSentCount', 'mailRoiAppointmentsCount', 'mailRoiAppointmentRatio', 'mailRoiDealsCount', 'mailRoiRatio'];
...
test('database metric labels are readable and stay on one line', () => {
  const filterGroupsCssSource = fs.readFileSync(path.join(__dirname, '../../assets/premium-database-filter-groups.css'), 'utf8');
