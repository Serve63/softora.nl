...
test('premium database page keeps customers fixed from Oisterwijk nearby to far away', () => {
  ...
  assert.match(pageSource, /assets\/premium-database-webdesign-action\.js\?v=20260503a/);
  ...
  assert.match(webdesignActionScriptSource, /async function preloadPhotoImages\(customers, limit\)/);
  ...
  assert.match(pageSource, /const bootPhotosReady = databaseHadBootstrapCustomers && state\.klanten\.length \? await loadCustomerPhotoMap\(state\.klanten\)\.then/);
  assert.match(pageSource, /state\.klanten = mergeCustomersWithPhotos\(state\.klanten, photoMap, state\.klanten\); renderPage\(\); return true;/);
  assert.match(pageSource, /if \(!bootPhotosReady\) await bootstrapCustomers\(\);/);
  assert.match(pageSource, /await webdesignActionController\.resumePendingJobs\(\)\.catch/);
  assert.match(pageSource, /state\.photoRestorePending = false; renderPage\(\); await webdesignActionController\.preloadPhotoImages\(getSortedCustomers\(getFilteredCustomers\(\)\), 24\); releaseDatabaseBootShell\(\);/);
  assert.match(pageSource, /if \(bootPhotosReady\) void bootstrapCustomers\(\)\.catch/);
