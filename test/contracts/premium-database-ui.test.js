...
test('premium database page keeps customers fixed from Oisterwijk nearby to far away', () => {
  ...
  assert.match(pageSource, /const bootPhotosReady = databaseHadBootstrapCustomers && state\.klanten\.length \? await Promise\.race\(\[/);
  assert.match(pageSource, /window\.setTimeout\(function \(\) \{ resolve\(false\); \}, 850\);/);
  ...
  assert.match(pageSource, /if \(!bootPhotosReady && !databaseHadBootstrapCustomers\) await bootstrapCustomers\(\);/);
  assert.match(pageSource, /state\.photoRestorePending = false; renderPage\(\); releaseDatabaseBootShell\(\); void webdesignActionController\.preloadPhotoImages\(getSortedCustomers\(getFilteredCustomers\(\)\), 10, 350\);/);
