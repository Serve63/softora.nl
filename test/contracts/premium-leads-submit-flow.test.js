...
test('premium leads modal submit flow has explicit timeout, error and finally handling', () => {
  ...
  assert.match(
    pageSource,
    /async function submitLeadToAgenda\(\) \{[\s\S]*setModalBusy\(true\);[\s\S]*setModalStatus\('Afspraak wordt in agenda gezet\.\.\.', ''\);[\s\S]*await postJsonWithFallback\([\s\S]*finalizeLeadMutation\(taskId\);[\s\S]*catch \(error\) \{[\s\S]*setModalStatus\(String\(error\?\.message \|\| 'Invoegen in agenda mislukt\.'\), 'error'\);[\s\S]*finally \{[\s\S]*setModalBusy\(false\);/s
  );
