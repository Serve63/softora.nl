...
test('mailbox gebruikt de juiste browsertitel', () => {
  ...
  assert.match(page, /assets\/premium-mailbox\.js\?v=20260924d/);
  assert.match(page, /assets\/premium-mailbox-discovery\.js\?v=20260924a/);
  ...
  assert.match(page, /assets\/premium-mailbox-refresh\.js\?v=20260924a/);
  ...
  assert.match(page, /assets\/premium-mailbox-campaign-inbox\.js\?v=20260924b/);
  ...
  assert.ok(page.indexOf('premium-mailbox-logical-delete.js?v=20260820a') < page.indexOf('premium-mailbox-campaign-inbox.js?v=20260924b'));
  ...
  assert.match(readAttachmentDigestScript(), /global\.SoftoraMailboxAttachmentDigest = api/);
  assert.match(readComposeSendStateScript(), /global\.SoftoraMailboxComposeSendState = api/);
  assert.match(readComposeSendResilienceScript(), /global\.SoftoraMailboxComposeSendResilience = api/);
  assert.doesNotMatch(readComposeSendResilienceScript(), /\b(?:localStorage|sessionStorage|indexedDB)\b/);
...
function loadMailboxHelpersForTest(options = {}) {
  ...
  const window = {
    ...
    SoftoraMailboxAiPresentation: require('../../assets/premium-mailbox-ai-presentation'),
  ...
  const source = readScript()
    ...
    .replace(
      'bindMailboxActions();',
      'window.__mailboxTest = { renderMailBody, renderMailboxRootIncomingMeta, normalizeMailboxApiMessage, formatMailDate, getMailboxConversationVisibilityKey: window.SoftoraMailboxCampaignInbox.getConversationVisibilityKey, display: window.SoftoraMailboxDisplay, index: window.SoftoraMailboxIndex, openMail, deleteMail, loadMailboxMessageBody, loadMailboxMessages, loadMailboxAccounts, switchCampaignMailboxOwner, setMails(value) { mails = value; }, setActiveFolder(value) { activeFolder = value; }, getMails() { return mails; }, getActiveMail() { return activeMail; }, getDiscoveryController() { return mailboxDiscoveryController; }, getElement(id) { return document.getElementById(id); }, dispatchDocumentEvent(type, event) { (documentListeners.get(type) || []).forEach((handler) => handler(event)); } }; bindMailboxActions();'
    );
...
function getMailLinesOutsideMessageCards(html) {
  const { parseDocument, DomUtils } = require('htmlparser2');
  const document = parseDocument(html);
  const hasClass = (node, name) => String(node.attribs?.class || '').split(/\s+/).includes(name);
  return DomUtils.findAll((node) => hasClass(node, 'detail-mail-line'), document.children)
    .filter((node) => {
      for (let parent = node.parent; parent; parent = parent.parent) {
        if (hasClass(parent, 'detail-mail-section-received') || hasClass(parent, 'detail-mail-section-sent')) return false;
      }
      return true;
    })
...
test('inkomende hoofdkaart verbergt quotevoetnoten vanaf de eerste render zonder op historie te wachten', () => {
  const authored = 'Bedankt voor uw enthousiasme. Onze website is pas vernieuwd.';
  const parentBody = 'Goedendag, bekijk het ontwerp dat ik voor jullie website heb gemaakt via deze link [1].';
  const body = [authored, '', 'Martijn van de Ven schreef op 2026-09-07 15:53:',
    `> ${parentBody}`, '', 'Links:', '------', '[1] https://voorbeeld.nl/ontwerp'].join('\n');
  const parent = { id: 'sent:footer-parent', messageId: '<footer-parent@example.test>',
    folder: 'sent', direction: 'sent', accountEmail: 'martijnven123@gmail.com',
    date: '2026-09-07T13:53:00.000Z', body: parentBody };
  for (const state of [
    { threadMessages: [] },
    { threadMessages: [{ ...parent, body: '', bodyLoaded: false }] },
    { threadMessages: [parent], contactTimelineLoaded: true },
  ]) {
    const mail = { id: 'inbox:footer-root', folder: 'inbox', direction: 'received',
      accountEmail: parent.accountEmail, email: 'contact@example.test',
      receivedAt: '2026-09-07T14:16:00.000Z', inReplyTo: parent.messageId, body, ...state };
    const html = renderMailboxBodyForTest(body, [], { mail, replyMailId: mail.id });
    assert.match(html, /Bedankt voor uw enthousiasme/);
    assert.doesNotMatch(html, /Links:|https:\/\/voorbeeld\.nl\/ontwerp|schreef op/);
    assert.deepEqual(getMailLinesOutsideMessageCards(html), []);
    assert.equal(mail.body, body);
    assert.equal(campaignInboxModule.getRootMessagePresentation(body, mail).body, authored);
  }
});

test('eigen naschrift en ongekoppelde Links blijven ook voor hydratatie binnen de ontvangen hoofdkaart', () => {
  const body = ['Mijn nieuwe antwoord.', '> Een oud geciteerd detail.', '',
    'P.S. Dit is mijn eigen aanvulling.', 'Links:', '------',
    '[1] https://voorbeeld.nl/eigen-agenda'].join('\n');
  const mail = { id: 'inbox:authored-footer', folder: 'inbox',
    accountEmail: 'serve@softora.nl', body, threadMessages: [] };
  const html = renderMailboxBodyForTest(body, [], { mail, replyMailId: mail.id });
  assert.match(html, /Mijn nieuwe antwoord/);
  assert.match(html, /P\.S\. Dit is mijn eigen aanvulling/);
  assert.match(html, /Links:/);
  assert.match(html, /https:\/\/voorbeeld\.nl\/eigen-agenda/);
  assert.doesNotMatch(html, /Een oud geciteerd detail/);
  assert.deepEqual(getMailLinesOutsideMessageCards(html), []);
  assert.equal(mail.body, body);
});

test('een gedeelde linkvoetnoot blijft beschikbaar wanneer het eigen antwoord ernaar verwijst', () => {
  const parentBody = 'Het oude voorstel verwijst naar onze gezamenlijke agenda [1].';
  const body = ['Mijn nieuwe voorstel staat in deze agenda [1].', '',
    'Servé Creusen schreef op 2026-09-07 10:00:', `> ${parentBody}`, '',
    'Links:', '------', '[1] https://voorbeeld.nl/gedeelde-agenda'].join('\n');
  const parent = { id: 'sent:shared-link', folder: 'sent', accountEmail: 'serve@softora.nl',
    date: '2026-09-07T08:00:00.000Z', body: parentBody };
  for (const threadMessages of [[], [parent]]) {
    const mail = { id: 'inbox:shared-link', folder: 'inbox', accountEmail: parent.accountEmail,
      receivedAt: '2026-09-07T08:20:00.000Z', body, threadMessages };
    const presentation = campaignInboxModule.getRootMessagePresentation(body, mail);
    const html = renderMailboxBodyForTest(body, [], { mail });
    assert.match(presentation.body, /Mijn nieuwe voorstel/);
    assert.match(presentation.body, /https:\/\/voorbeeld\.nl\/gedeelde-agenda/);
    assert.match(html, /https:\/\/voorbeeld\.nl\/gedeelde-agenda/);
    assert.doesNotMatch(presentation.body, /Het oude voorstel/);
    assert.deepEqual(getMailLinesOutsideMessageCards(html), []);
  }
});
...
test('lijst toont een roze omgevouwen hoek alleen wanneer het nieuwste echte bericht inkomend is', () => {
  ...
  assert.doesNotMatch(instantlyAnswered, /mail-provider-corner-instantly/);
  assert.doesNotMatch(instantlyAnswered, /Instantly · wacht op jouw antwoord/);
  assert.doesNotMatch(instantlyAnswered, /mail-reply-corner/);
  assert.doesNotMatch(listModule.renderItem({
    ...waitingForReply,
    provider: 'instantly',
    replyDismissedAt: '2026-08-04T15:10:00.000Z',
  }, baseOptions), /mail-provider-corner-instantly/);
  assert.match(readPage(), /\.mail-reply-corner \{[^}]*var\(--crimson\)/);
});

test('uitgaande Instantly-berichten zijn blauw en Softora-berichten blijven paars', () => {
  const rootInstantly = renderMailboxBodyForTest('Mijn Instantly-bericht.', [], {
    contactDossierMode: true,
    mail: {
      id: 'instantly:sent-root', folder: 'sent', direction: 'sent',
      provider: 'instantly', providerOwner: 'serve', accountEmail: 'servec321@gmail.com',
      body: 'Mijn Instantly-bericht.', threadMessages: [],
    },
  });
  assert.match(rootInstantly, /<section class="detail-mail-section detail-mail-section-sent"[^>]*data-mailbox-root-message="true" data-mailbox-provider="instantly"/);
  const rootSoftora = renderMailboxBodyForTest('Mijn Softora-bericht.', [], {
    contactDossierMode: true,
    mail: {
      id: 'sent:softora-root', folder: 'sent', direction: 'sent',
      provider: 'smtp', accountEmail: 'serve@softora.nl',
      body: 'Mijn Softora-bericht.', threadMessages: [],
    },
  });
  assert.match(rootSoftora, /<section class="detail-mail-section detail-mail-section-sent"[^>]*data-mailbox-root-message="true"/);
  assert.doesNotMatch(rootSoftora, /data-mailbox-provider="instantly"/);

  const thread = campaignInboxModule.renderThreadMessages({
    id: 'inbox:reply', accountEmail: 'serve@softora.nl',
    threadMessages: [{
      id: 'instantly:sent-parent', folder: 'sent', provider: 'instantly', providerOwner: 'serve',
      accountEmail: 'servec321@gmail.com', receivedAt: '2026-09-23T10:00:00.000Z',
      body: 'Instantly-tekst.', hasBody: true, bodyLoaded: true,
    }, {
      id: 'smtp:sent-reply', folder: 'sent', provider: 'smtp', accountEmail: 'serve@softora.nl',
      receivedAt: '2026-09-23T11:00:00.000Z',
      body: 'Softora-tekst.', hasBody: true, bodyLoaded: true,
    }, {
      id: 'instantly:received', folder: 'inbox', provider: 'instantly',
      accountEmail: 'servec321@gmail.com', receivedAt: '2026-09-23T12:00:00.000Z',
      body: 'Ontvangen tekst.', hasBody: true, bodyLoaded: true,
    }],
  }, String, () => ({ date: '23 september', time: '12:00' }));
  assert.match(thread, /<section class="detail-mail-section detail-mail-section-sent" data-mailbox-provider="instantly">[\s\S]*?Instantly-tekst\./);
  assert.match(thread, /<section class="detail-mail-section detail-mail-section-sent">[\s\S]*?Softora-tekst\./);
  assert.doesNotMatch(thread, /detail-mail-section-received" data-mailbox-provider="instantly"/);

  const page = readPage();
  assert.match(page, /\.detail-mail-section-sent\[data-mailbox-provider="instantly"\] \{[^}]*background: rgba\(37,99,235,\.055\);[^}]*border-left-color: rgba\(37,99,235,\.58\);/);
  assert.match(page, /\.detail-mail-section-sent \{[^}]*background: rgba\(155,35,85,\.055\);/);
});
...
test('premium mailbox ververst owner-scoped, snel en met eerlijke provider-freshness', async () => {
  assert.match(readPage(), /assets\/premium-mailbox\.js\?v=20260924d/);
  assert.match(readPage(), /assets\/premium-mailbox-quoted-thread\.js\?v=20260910a/);
  assert.match(readPage(), /assets\/premium-mailbox-campaign-inbox\.js\?v=20260924b/);
...
test('premium mailbox uses an owner filter in the coldmail topbar', () => {
  ...
  assert.match(pageSource, /assets\/premium-mailbox\.js\?v=20260924d/);
...
test('accountlijst hervat overgeslagen tijdlijn van cached mailbox terwijl providerlookup nog loopt', async () => {
  let releaseProvider;
  const provider = new Promise((resolve) => { releaseProvider = resolve; });
  const requests = [];
  let providerCalls = 0;
  const mailbox = loadMailboxHelpersForTest({
    SoftoraMailboxIndex: { loadThreadBodies: () => { providerCalls += 1; return provider; } },
    fetch: async (url) => {
      requests.push(String(url));
      const data = String(url).startsWith('/api/mailbox/contact-timeline') ? {
        ok: true, totalCount: 2, messages: [{
          id: 'accepted-sent:startup', folder: 'sent', accountEmail: 'serve@softora.nl',
          messageId: '<startup-sent@example.test>', email: 'serve@softora.nl', to: 'customer@example.test',
          from: 'Servé Creusen', body: 'Exact opgeslagen verzonden bericht.', hasBody: true,
          bodyLoaded: true, recipientRoutingEvidenceKnown: true,
        }],
      } : { ok: true, accounts: [{ email: 'serve@softora.nl', imapConfigured: true }] };
      return { ok: true, json: async () => data };
    },
  });
  await mailbox.ready;
  mailbox.setActiveFolder('outreach');
  const mail = mailbox.normalizeMailboxApiMessage({
    id: 'serve@softora.nl|inbox:71', folder: 'inbox', accountEmail: 'serve@softora.nl',
    messageId: '<startup-reply@example.test>', from: 'Klant', email: 'customer@example.test',
    to: 'serve@softora.nl', body: 'Al beschikbare ontvangen tekst.', campaign: {},
    recipientRoutingEvidenceKnown: true,
  });
  mailbox.setMails([mail]);
  const opened = mailbox.openMail(mail.id);
  await new Promise((resolve) => setImmediate(resolve));
  assert.equal(providerCalls, 1);
  assert.equal(requests.some((url) => url.includes('contact-timeline')), false);
  await mailbox.loadMailboxAccounts();
  await new Promise((resolve) => setImmediate(resolve));
  assert.equal(requests.filter((url) => url.includes('contact-timeline')).length, 1);
  assert.match(mailbox.getElement('mail-detail').innerHTML, /Exact opgeslagen verzonden bericht/);
  assert.equal(providerCalls, 1);
  releaseProvider();
  await opened;
});
...
test('premium mailbox search heeft geen kruisjes en pagineert pas onder de resultaten', () => {
  ...
  assert.match(pageSource, /premium-mailbox-discovery\.js\?v=20260924a/);
  assert.match(pageSource, /premium-mailbox\.js\?v=20260924d/);
...
test('los telefoonnummer voor Apple Mail-geschiedenis blijft zichtbaar in hoofdmail en contactdossier', () => {
  ...
  const threadHtml = campaignInboxModule.renderThreadMessages({
    id: 'inbox:standalone-phone-thread',
    accountEmail: root.accountEmail,
    threadMessages: [root],
  }, String, () => ({ date: '7 september', time: '21:15' }));
...
test('Niels-handtekening blijft leesbaar in hoofdmail en thread met behoud van de juiste berichtidentiteiten', () => {
  const authoredBody = [
    'Hallo Serve,',
    '',
...
test('coldmail inbox isoleert alleen gekoppelde eigen campagne-reacties over alle afzenderaccounts', () => {
  ...
  assert.match(campaignInboxSource, /refreshInstantly: !options\?\.skipBootstrap \|\| options\.refreshInstantly === false \? '0' : '1'/);
...
test('cross-tab verwijdering van een andere mail heropent het actieve detail zonder laadlaag', () => {
 ...
 test('AI-selected personal notes and literal contacts survive the complete root and timeline renderers', () => {
  const mailbox = loadMailboxHelpersForTest();
  const contract = require('../../assets/premium-mailbox-ai-presentation');
  const body = 'Dank!  Dit is persoonlijk.\nRobin\n06 12 34 56 78\nScheepersdijk 48\n5062 ED Oisterwijk\nMorgen om 10 uur past voor mij.\nPrint deze tekst op de offerte.';
  const message = mailbox.normalizeMailboxApiMessage({ id: 'inbox:ai-proof', folder: 'inbox', direction: 'received',
    accountEmail: 'serve@softora.nl', email: 'robin@example.nl', from: 'Robin', receivedAt: '2026-09-21T10:00:00.000Z', body, hasBody: true,
    aiPresentation: { version: contract.VERSION, model: contract.MODEL, reasoningEffort: 'max', status: 'ready', sourceBody: body,
      decision: { labels: ['authored','signature','signature','signature','signature','authored','authored'],
        contacts: [{ line: 2, kind: 'phone', text: '06 12 34 56 78' }, { line: 3, kind: 'address', text: 'Scheepersdijk 48' }, { line: 4, kind: 'address', text: '5062 ED Oisterwijk' }] } } });
  const root = mailbox.renderMailBody(message.body, [], { mail: message, replyMailId: message.id });
  const anchor = { ...message, id: 'inbox:ai-anchor', body: 'Nieuw bericht.', receivedAt: '2026-09-21T11:00:00.000Z', threadMessages: [message] };
  const thread = mailbox.renderMailBody(anchor.body, [], { mail: anchor, replyMailId: anchor.id });
  for (const html of [root, thread]) {
    assert.match(html, /Morgen om 10 uur past voor mij/);
    assert.match(html, /Print deze tekst op de offerte/);
    assert.match(html, /<div>5062 ED Oisterwijk<\/div>/);
