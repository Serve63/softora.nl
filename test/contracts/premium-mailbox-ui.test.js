...
test('premium mailbox uses a mailbox account dropdown in the topbar', () => {
  ...
  assert.match(pageSource, /\/api\/mailbox\/accounts/);
  assert.match(pageSource, /\/api\/mailbox\/messages\?account=/);
  assert.match(pageSource, /\/api\/mailbox\/send/);
  assert.match(pageSource, /async function loadMailboxAccounts\(\)/);
  assert.match(pageSource, /async function loadMailboxMessages\(\)/);
  assert.match(pageSource, /async function sendMail\(\)/);
  assert.match(pageSource, /function getMailboxAccounts\(\) \{\s*return mailboxAccounts\.map\(\(account\) => account\.email\);\s*\}/);
  ...
  assert.match(pageSource, /async function applyMailboxAccount\(email\) \{[\s\S]*activeMailboxAccount = email;[\s\S]*setMailboxAccountUi\(email\);/);
