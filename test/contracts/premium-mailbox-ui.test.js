...
test('mailbox hydrateert een oorspronkelijke webdesignlink uit exact MIME-bewijs', async () => {
  ...
  assert.match(
    html,
    /<a href="https:\/\/www\.softora\.nl\/webdesign\/salon-tof\?cid=safe-row-247&amp;sender=serve" target="_blank" rel="noopener noreferrer">deze link<\/a>/
  );
});
...
test('premium mailbox ververst handmatig en automatisch iedere vijf minuten', async () => {
  assert.match(readPage(), /assets\/premium-mailbox\.js\?v=20260724b/);
...
test('premium mailbox ruimt technische mail-links op voor weergave', () => {
  ...
  assert.match(scriptSource, /<div class="detail-body-text">\$\{window\.SoftoraMailboxDisplay\.renderDetailBody\(m, renderMailBody\(detailBody, detailBodyImages,/);
