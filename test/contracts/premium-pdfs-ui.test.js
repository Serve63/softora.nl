...
test('premium pdf builder scales the live preview to the available viewport', () => {
  ...
  assert.match(pageSource, /--preview-overscan:\s*1\.08;/);
  ...
  assert.match(pageSource, /function fitPreviewToViewport\(\) \{[\s\S]*const availableWidth = Math\.max\(240, stage\.clientWidth\);[\s\S]*const maxScale = Number\.parseFloat\(getComputedStyle\(document\.documentElement\)\.getPropertyValue\('--preview-max-scale'\)\) \|\| 1;[\s\S]*const overscan = Number\.parseFloat\(getComputedStyle\(document\.documentElement\)\.getPropertyValue\('--preview-overscan'\)\) \|\| 1;[\s\S]*const scale = Math\.min\(maxScale, \(availableWidth \/ A4_PREVIEW_WIDTH\) \* overscan\);/);
