...
test('premium pdf builder scales the live preview to the available viewport', () => {
  ...
  assert.match(pageSource, /\.preview-stage \{[\s\S]*overflow:\s*auto;[\s\S]*padding:\s*0;[\s\S]*justify-content:\s*center;/);
  ...
  assert.match(pageSource, /function fitPreviewToViewport\(\) \{[\s\S]*const availableWidth = Math\.max\(240, stage\.clientWidth\);[\s\S]*const maxScale = Number\.parseFloat\(getComputedStyle\(document\.documentElement\)\.getPropertyValue\('--preview-max-scale'\)\) \|\| 1;[\s\S]*const scale = Math\.min\(maxScale, availableWidth \/ A4_PREVIEW_WIDTH\);/);
