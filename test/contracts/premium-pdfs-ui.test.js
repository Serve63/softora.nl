...
test('premium pdf builder scales the live preview to the available viewport', () => {
  ...
  assert.doesNotMatch(pageSource, /data-sidebar-key="pdfs"|href="\/premium-pdfs"[^>]*sidebar-link/);
  ...
  assert.match(pageSource, /Werkwijze bij nieuwe opdracht/);
  assert.match(pageSource, /Akkoord met de offerte en algemene voorwaarden\./);
  assert.match(builderSource, /const DEFAULT_OFFERTE_NOTITIES = 'In de bijlage vind je de offerte en de algemene voorwaarden van Softora VOF\. Door akkoord te geven op deze offerte verklaar je ook akkoord te gaan met de algemene voorwaarden\. Reageer bij akkoord altijd expliciet met: "Akkoord met de offerte en algemene voorwaarden\."';/);
  assert.match(builderSource, /id:'notities'[\s\S]*default:\s*DEFAULT_OFFERTE_NOTITIES/);
  assert.match(builderSource, /const DEFAULT_FACTUUR_NOTITIES = 'Op deze factuur en de onderliggende opdracht zijn de algemene voorwaarden en, indien van toepassing, de verwerkersovereenkomst van Softora VOF van toepassing, zoals vóór akkoord verstrekt en geaccepteerd bij akkoord op de offerte\/opdrachtbevestiging\.';/);
  assert.match(builderSource, /factuur:\s*\[[\s\S]*label:'Notities \(optioneel\)'[\s\S]*default:\s*DEFAULT_FACTUUR_NOTITIES/);
  assert.match(builderSource, /const DEFAULT_OPLEVERINGSMAIL_NOTITIES = 'Je hebt 7 kalenderdagen om concrete gebreken te melden\. Daarna geldt het project als geaccepteerd\.';/);
  assert.match(builderSource, /opleveringsmail:\s*\[[\s\S]*label:'Standaard opmerking'[\s\S]*default:\s*DEFAULT_OPLEVERINGSMAIL_NOTITIES/);
  assert.match(builderSource, /docType === 'opleveringsmail'[\s\S]*<div class="pdf-doctype">Opleveringsmail<\/div>/);
  assert.match(builderSource, /docType==='opleveringsmail'\?'OPLEVERINGSMAIL'/);
  ...
  assert.match(builderSource, /function fmtEur\(n\) \{ return '€\\u00a0'/);
  assert.match(builderSource, /function fitPreviewToViewport\(\) \{[\s\S]*const availableWidth = Math\.max\(240, stage\.clientWidth - 48\);[\s\S]*const availableHeight = Math\.max\(320, stage\.clientHeight - 36\);[\s\S]*const maxScale = Number\.parseFloat\(getComputedStyle\(document\.documentElement\)\.getPropertyValue\('--preview-max-scale'\)\) \|\| 1;[\s\S]*const scale = Math\.min\(maxScale, availableWidth \/ A4_PREVIEW_WIDTH, availableHeight \/ A4_PREVIEW_HEIGHT\);/);
  assert.match(builderSource, /const hcols\s+= \{ 1:'#d97706', 2:'#b45a00', 3:'#c0392b' \};/);
  assert.match(builderSource, /const hcols=\{1:\[217,119,6\],2:\[180,90,0\],3:\[192,57,43\]\};/);
  assert.match(builderSource, /function setupPreviewAutoFit\(\) \{[\s\S]*new ResizeObserver\(\(\) => fitPreviewToViewport\(\)\);/);
  assert.match(builderSource, /function bindPdfBuilderActions\(\) \{[\s\S]*setDoc\(tab\.dataset\.doc, tab\);[\s\S]*downloadButton\.addEventListener\('click', downloadPDF\);/);
  assert.match(builderSource, /bindPdfBuilderActions\(\);\s*buildForm\(\);\s*setupPreviewAutoFit\(\);\s*fitPreviewToViewport\(\);/);
  assert.doesNotMatch(pageSource, /onclick=/);
  assert.doesNotMatch(pageSource, /function buildForm|function downloadPDF|const docConfigs/);
