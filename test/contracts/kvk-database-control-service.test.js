...
function createInMemoryService() {
  ...
  return { service, getStoredRow: (stateKey) => storedRows.get(stateKey) || null };
}
...
test('worker poll and report require the sync token and preserve enabled', async () => {
  const { service } = createInMemoryService();
  ...
  assert.equal(report.payload.control.workerState, 'running');
