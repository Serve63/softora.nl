...
test('data ops store saves webdesign and device mockup as one photo record', async () => {
  ...
  const result = await store.uploadDesignPhoto(
    {
      ...
      websiteMockupName: 'softora.nl-device-mockup-v12.jpg',
      legacyMeta: {
  ...
  assert.equal(upserts[0].row.legacy_meta.websiteMockupName, 'softora.nl-device-mockup-v12.jpg');
  assert.equal(upserts[0].row.legacy_meta.mockup.fileName, 'softora.nl-device-mockup-v12.jpg');
  ...
  assert.equal(upserts[0].row.legacy_meta.mockup.renderer, 'softora-browser-device-v12');
