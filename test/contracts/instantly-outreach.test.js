...
test('safe Instantly upload prepares CSV only after reserving leads and permanent guards', async () => {
  ...
  assert.equal(result.campaignSubjectTemplate, '{{SfSubject}}');
  assert.ok(result.csvHeaders.every((header) => header.length <= 20));
  ...
  assert.match(result.campaignHtmlTemplate, /<img[^>]+src="\{\{SfImage\}\}"/);
  assert.match(result.campaignHtmlTemplate, /<img[^>]+src="\{\{SfMockup\}\}"/);
  assert.match(result.campaignHtmlTemplate, /href="\{\{SfPreview\}\}"/);
  ...
  assert.match(result.campaignHtmlTemplate, /\{\{SfSender\}\}/);
  assert.match(result.campaignTemplateInstructions, /Gebruik in Instantly als onderwerp: \{\{SfSubject\}\}/);
  assert.match(result.campaignTemplateInstructions, /map de Sf-kolommen als Custom Variable/i);
