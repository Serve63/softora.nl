...
test('safe Instantly upload prepares CSV only after reserving leads and permanent guards', async () => {
  ...
  assert.match(result.csv, /^EMAIL,CONTACT,COMPANY,WEBSITE,PERSONALIZATION,PHONE NUMBER/);
  ...
  assert.match(result.campaignHtmlTemplate, /href="\{\{softora_unsubscribe_url\}\}"/);
...
test('instantly sync can refresh existing lead variables without adding duplicate leads', async () => {
  ...
  const body = JSON.parse(fetchCalls[0].options.body);
  assertInstantlyHtmlUsesVisibleWebdesignImages(body.personalization);
