...
test('sportschool logboek page is available as installable pretty page', () => {
  ...
  assert.match(pageSource, /assets\/sportschool-logboek\.css\?v=20260806a/);
  ...
  assert.match(pageSource, /assets\/sportschool-logboek-state\.js\?v=20260806a/);
  assert.match(pageSource, /assets\/sportschool-logboek\.js\?v=20260806a/);
  ...
  assert.match(scriptSource, /trainingDayCompletions/);
  assert.match(scriptSource, /trainingDateKey/);
  assert.match(scriptSource, /dataset\.exerciseComplete/);
  assert.match(scriptSource, /aria-pressed/);
  assert.match(scriptSource, /writeExerciseCompletion/);
  assert.match(stateScriptSource, /mergeExerciseSource/);
  assert.match(stateScriptSource, /reconcileExerciseSources/);
  assert.match(stateScriptSource, /normalizeTrainingDayCompletions/);
  assert.match(stateScriptSource, /updateTrainingExerciseCompletion/);
  ...
  assert.match(stylesSource, /\.exercise-card\.is-completed\s*\{[\s\S]*?rgba\(34, 197, 94, 0\.13\)/);
  assert.match(stylesSource, /\.exercise-complete\[aria-pressed='true'\]\s*\{[\s\S]*?background: #16a34a;/);
