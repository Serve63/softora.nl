...
test('sportschool logboek page is available as installable pretty page', () => {
  ...
  assert.match(scriptSource, /setCurrentFormStatus/);
  assert.match(scriptSource, /formHistory/);
  assert.match(scriptSource, /formSessions/);
  assert.match(scriptSource, /formSessions\[current\.exerciseKey\]/);
  assert.match(scriptSource, /buildFormSessionSlots/);
  assert.match(scriptSource, /storageDay\(day\) === currentWeekday\(\)/);
  assert.match(scriptSource, /button\.disabled = !session\.isCurrent/);
  assert.match(scriptSource, /setCurrentFormStatus\(day, exercise\.order, nextStatus\)/);
  assert.match(stateScriptSource, /setFormSessionStatus/);
  assert.match(stateScriptSource, /slice\(-FORM_HISTORY_LENGTH\)/);
  assert.match(bootstrapScriptSource, /formSessions: \{ \.\.\.remoteFormSessions, \.\.\.localFormSessions \}/);
  assert.match(scriptSource, /form-status-button/);
  assert.match(scriptSource, /nextFormStatus\(button\.dataset\.status\)/);
  assert.match(scriptSource, /createFormStatusVisual/);
  assert.match(scriptSource, /createElementNS\(svgNamespace, 'svg'\)/);
  assert.match(scriptSource, /button\.replaceChildren\(createFormStatusVisual\(currentStatus\)\)/);
  ...
  assert.match(stylesSource, /\.gym-app\s*\{[\s\S]*?width: min\(100%, 400px\);/);
  assert.match(stylesSource, /\.gym-header\s*\{[\s\S]*?min-height: calc\(80px \+ var\(--safe-top\)\);/);
  assert.match(stylesSource, /\.gym-section\s*\{[\s\S]*?padding: 16px 14px calc\(22px \+ var\(--safe-bottom\)\);/);
  assert.match(stylesSource, /\.icon-button\s*\{[\s\S]*?width: 36px;[\s\S]*?height: 36px;/);
  assert.match(stylesSource, /\.exercise-card\s*\{[\s\S]*?padding: 6px 8px;/);
  assert.match(stylesSource, /\.exercise-list\s*\{[\s\S]*?gap: 4px;/);
  assert.match(stylesSource, /\.metric\s*\{[\s\S]*?width: 32px;[\s\S]*?min-height: 32px;/);
  assert.match(stylesSource, /\.metric-kg\s*\{[\s\S]*?width: 40px;/);
  assert.match(stylesSource, /\.metric-input\s*\{[\s\S]*?width: 100%;[\s\S]*?height: 19px;[\s\S]*?padding: 0;/);
  assert.match(stylesSource, /\.form-history\s*\{/);
  assert.match(stylesSource, /\.exercise-top\s*\{[\s\S]*?grid-template-rows: 34px minmax\(30px, auto\);[\s\S]*?row-gap: 1px;/);
  ...
  assert.match(stylesSource, /\.form-status:nth-child\(3\)\s*\{[\s\S]*?width: 40px;/);
  ...
  assert.match(stylesSource, /\.form-status-button\s*\{[\s\S]*?height: 26px;/);
  assert.match(stylesSource, /\.form-status-icon\s*\{[\s\S]*?width: 18px;[\s\S]*?height: 18px;/);
  ...
  assert.match(stylesSource, /\.exercise-title\s*\{[\s\S]*?font-size: 12px;/);
  ...
  assert.match(stylesSource, /\.exercise-notes\s*\{[\s\S]*?grid-column: 1 \/ span 2;[\s\S]*?grid-row: 2;[\s\S]*?min-height: 24px;[\s\S]*?overflow: hidden;[\s\S]*?resize: none;[\s\S]*?font-size: 9\.5px;[\s\S]*?overflow-wrap: anywhere;[\s\S]*?white-space: pre-wrap;/);
