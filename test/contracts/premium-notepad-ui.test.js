...
test('premium kladblok gebruikt dezelfde layout-ruggengraat en titelmaat als dashboard', () => {
  ...
  assert.match(scriptSource, /var loadReady = false;/);
  assert.match(scriptSource, /var dirty = false;/);
  assert.match(scriptSource, /editor\.setAttribute\("contenteditable", "false"\)/);
  assert.match(scriptSource, /async function loadRemoteStateWithRetry\(\)/);
  assert.match(scriptSource, /return await getUiStateClient\(\)\.get\(REMOTE_SCOPE\)/);
  ...
  assert.match(scriptSource, /loadReady = true;/);
  assert.match(scriptSource, /setStatus\(text \? "Notities geladen\." : "Kladblok is leeg\.", ""\)/);
  assert.match(scriptSource, /if \(!loadReady \|\| !dirty\) return;/);
  assert.match(scriptSource, /if \(loadReady && dirty\) void save\(\);/);
  assert.match(scriptSource, /Niets is overschreven/);
