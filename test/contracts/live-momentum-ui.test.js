...
test('live momentum page renders the requested dashboard surface', () => {
  ...
  assert.match(html, /href="\/assets\/live-momentum\.css\?v=20260716h"/);
  ...
  assert.match(html, /<script src="\/assets\/live-momentum\.js\?v=20260716c" defer><\/script>/);
...
test('live momentum stylesheet keeps the visual replica self-contained', () => {
  ...
  assert.match(css, /\.line-stage\s*\{[\s\S]*position:\s*relative;[\s\S]*isolation:\s*isolate;/);
  ...
  assert.match(css, /\.line-grid\s*\{[\s\S]*z-index:\s*0;/);
  assert.match(css, /\.line-segment\s*\{[\s\S]*z-index:\s*1;/);
  assert.match(css, /\.line-point\s*\{[\s\S]*z-index:\s*3;/);
  assert.match(css, /\.line-point-label\s*\{[\s\S]*z-index:\s*4;[\s\S]*background:\s*var\(--page-bg\);[\s\S]*box-shadow:\s*0 0 0 6px var\(--page-bg\);/);
  assert.match(css, /\.line-point-label\.is-above\s*\{[\s\S]*var\(--line-label-offset, 16px\)/);
  assert.match(css, /\.line-point-label\.is-below\s*\{[\s\S]*var\(--line-label-offset, 16px\)/);
...
test('live momentum script wires habit toggles to chart and persisted state', () => {
  ...
  assert.match(js, /const safeLabelX = Math\.min\(Math\.max\(x, 30\), viewWidth - 30\);/);
  assert.match(js, /label\.style\.setProperty\('--line-label-offset', `\$\{16 \+ \(\(index % 2\) \* 16\)\}px`\);/);
  assert.match(js, /stage\.append\(point, label\);/);
  assert.doesNotMatch(js, /point\.append\(label\);/);
