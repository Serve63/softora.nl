...
const endGameCardFiles = [
  ...
  'sertraline-vrij.png', 'serves-gezondheidsdossier.png', 'tanden-rechtzetten.png', 'tandenbleek-voorraad.png',
...
test('live momentum page renders the requested dashboard surface', () => {
  ...
  assert.match(html, /href="\/assets\/live-momentum-video\.css\?v=20260720a"/);
  ...
  assert.match(html, /<script src="\/assets\/live-momentum-endgame-cards\.js\?v=20260720a" defer><\/script>/);
  assert.match(html, /<script src="\/assets\/live-momentum-video\.js\?v=20260720a" defer><\/script>/);
...
test('live momentum script wires habit toggles to chart and persisted state', () => {
  ...
  assert.match(endGameCardsJs, /title:\s*'Sertraline vrij'/);
  ...
  [
    ...
    'Sertraline vrij', '2030...', 'Oktober 2024…'
  ...
  assert.match(videoJs, /interactionLayer\.setAttribute\('aria-hidden', 'true'\)/);
  assert.match(videoJs, /interactionLayer\.addEventListener\('click', activateVideo\)/);
  assert.match(videoJs, /event:\s*'listening'/);
  assert.match(videoJs, /sendPlayerCommand\('playVideo'\)/);
  assert.match(videoJs, /sendPlayerCommand\('unMute'\)/);
  assert.match(videoJs, /sendPlayerCommand\('setVolume', \[100\]\)/);
  assert.match(videoJs, /function activateVideo\(\)\s*\{[\s\S]*player\.classList\.remove\('is-ready'\);[\s\S]*scheduleReveal\(\);/);
  assert.match(videoJs, /player\.classList\.add\('is-ready'\)/);
  assert.match(videoJs, /}, 5200\)/);
  assert.doesNotMatch(videoJs, /pauseVideo|seekTo|togglePlayback|onStateChange/);
  assert.match(videoCss, /\.momentum-video-stage iframe\s*\{[\s\S]*top:\s*-23%;[\s\S]*left:\s*0;[\s\S]*width:\s*100%;[\s\S]*height:\s*146%;[\s\S]*opacity:\s*0;[\s\S]*pointer-events:\s*none;/);
  assert.match(videoCss, /\.momentum-video-stage\.is-ready iframe\s*\{[\s\S]*opacity:\s*1;/);
  assert.match(videoCss, /\.momentum-video-interaction\s*\{[\s\S]*position:\s*absolute;[\s\S]*inset:\s*0;[\s\S]*background:\s*transparent;[\s\S]*touch-action:\s*none;/);
  assert.match(videoJs, /autoplay:\s*'1'/);
  assert.match(videoJs, /mute:\s*'1'/);
