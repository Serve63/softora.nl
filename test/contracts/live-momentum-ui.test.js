...
test('live momentum page renders the requested dashboard surface', () => {
  ...
  assert.match(html, /href="\/assets\/live-momentum\.css\?v=20260716k"/);
  ...
  assert.match(html, /<script src="\/assets\/live-momentum\.js\?v=20260716e" defer><\/script>/);
  ...
  assert.equal((html.match(/class="end-game-goal-card"/g) || []).length, 10);
...
test('live momentum stylesheet keeps the visual replica self-contained', () => {
  ...
  assert.match(css, /\.end-game-goals\s*\{[\s\S]*overflow-x:\s*auto;[\s\S]*scroll-snap-type:\s*x proximity;/);
