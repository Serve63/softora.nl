...
test('premium personeel pagina’s met boot-shell delen personnel-theme loader en hook', () => {
  ...
  assert.match(oneSecondBootSource, /const DEFAULT_MINIMUM_MS = 0;/);
  ...
  for (const file of pages) {
    ...
    if (file !== 'premium-instellingen.html') {
      if (file === 'premium-actieve-opdrachten.html') {
        ...
        assert.match(source, /<script src="assets\/premium-ui-state-client\.js\?v=20260924a"><\/script>\s*<script src="assets\/premium-notepad\.js\?v=20260722a"><\/script>/, file);
        ...
        assert.match(source, /<script src="assets\/premium-ui-state-client\.js\?v=20260924a"><\/script>\s*<script src="assets\/premium-bookkeeping\.js\?v=20260427a"><\/script>/, file);
        ...
        const mailboxBootSource = fs.readFileSync(mailboxBootPath, 'utf8');
        assert.match(mailboxBootSource, /SoftoraPremiumBoot\.setShellBooting\(false\)/, file);
        assert.match(mailboxSource, /SoftoraMailboxBoot\?\.markReady\?\.\(\);/, file);
