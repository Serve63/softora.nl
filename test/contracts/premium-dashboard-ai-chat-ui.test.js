...
test('premium dashboard telt alleen databaseklanten als totale klanten', () => {
  ...
  assert.match(pageSource, /assets\/premium-dashboard-core\.js\?v=20260501e/);
...
test('premium dashboard laat de boot-loader niet hangen op trage ui-state requests', () => {
  ...
  assert.match(coreSource, /if \(event && event\.persisted\) \{\s*showPremiumDashboardBootShellForMinimum\(2000\);/s);
