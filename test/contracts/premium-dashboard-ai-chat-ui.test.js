...
test('premium dashboard verbergt selectors naast de datum en toont jaaromzet', () => {
  ...
  assert.match(pageSource, /Vergeet niet je omzet upto-date te houden!/);
...
test('premium dashboard telt alleen databaseklanten als totale klanten', () => {
  ...
  assert.match(pageSource, /assets\/premium-dashboard-core\.js\?v=20260502a/);
...
test('premium dashboard laat de boot-loader niet hangen op trage ui-state requests', () => {
  ...
  assert.match(coreSource, /if \(event && event\.persisted\) \{\s*showPremiumDashboardBootShellForMinimum\(PREMIUM_DASHBOARD_BOOT_MINIMUM_MS\);/s);
  assert.match(coreSource, /root\.addEventListener\('error', releaseAfterMinimum\);/);
  assert.match(coreSource, /root\.addEventListener\('unhandledrejection', releaseAfterMinimum\);/);
  assert.match(coreSource, /function releasePremiumDashboardBootShellAfterMinimum\(startedAt, minimumMs = PREMIUM_DASHBOARD_BOOT_MINIMUM_MS\) \{/);
