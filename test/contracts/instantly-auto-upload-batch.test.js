...
test('automatic Instantly mail-ready batching stays bounded, drains both owners and preserves safety stops', async () => {
  assert.equal(DEFAULT_AUTO_UPLOAD_BATCH_SIZE, 10);
  assert.equal(MAX_AUTO_UPLOAD_BATCH_SIZE, 25);
  assert.equal(resolveAutoUploadBatchLimit({}, { batchSize: 10 }), 10);
  assert.equal(resolveAutoUploadBatchLimit({}, { batchSize: 40 }), 25);
  assert.equal(resolveAutoUploadBatchLimit({ limit: 7 }, { batchSize: 10 }), 7);
  assert.equal(resolveAutoUploadBatchLimit({ limit: 999 }, { batchSize: 10 }), 25);

  {
    ...
    assert.equal(result.batchLimit, 10);
