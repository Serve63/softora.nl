...
test('premium actieve opdrachten start direct met server-bootstrap zonder kunstmatige wachttijd', () => {
  ...
  assert.doesNotMatch(scriptSource, /if \(hadBootstrap\) void loadRemoteUiState/);
  assert.match(scriptSource, /setBuildMode\(buildMode, \{ persist: false \}\)/);
});
...
test('premium opdrachtdossier laadt eerst een bestaand cache-item voordat opus opnieuw genereert', () => {
  ...
  assert.match(pageSource, /<!-- SOFTORA_PAGE_STATE_BOOTSTRAP --><script src="assets\/premium-ui-state-client\.js\?v=20260922a"><\/script>/);
