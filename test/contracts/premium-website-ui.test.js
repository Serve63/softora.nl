...
test('premium website houdt Van Idee Naar Lancering op desktop op één regel', () => {
  ...
  assert.match(source, /@media \(min-width: 769px\) \{[\s\S]*#werkwijze \.werkwijze-grid\s*\{[\s\S]*grid-template-columns:\s*minmax\(0,\s*1\.45fr\) minmax\(300px,\s*0\.55fr\);/);
...
test('premium website laat werkwijze titel natuurlijk wrappen op mobiel', () => {
  ...
  assert.match(
    source,
    /@media \(min-width: 769px\) \{[\s\S]*#werkwijze \.werkwijze-grid\s*\{[\s\S]*grid-template-columns:\s*minmax\(0,\s*1\.45fr\) minmax\(300px,\s*0\.55fr\);/
  );
});
...
test('premium website footer bevat geen publieke personeel-login link meer', () => {
  const filePath = path.join(__dirname, '../../premium-website.html');
  const source = fs.readFileSync(filePath, 'utf8');

  assert.doesNotMatch(source, /\/premium-personeel-login\?logout=1/);
  assert.doesNotMatch(source, /\.footer-legal a\[href\*="premium-personeel-login"\]/);
});
