...
test('premium website toont een speelse krulpijl van hero richting wat we bouwen', () => {
  ...
  assert.match(source, /\.diensten-arrow-wrap\s*\{[\s\S]*left:\s*clamp\(-20rem,\s*-15vw,\s*-10rem\);[\s\S]*top:\s*-17\.5rem;[\s\S]*width:\s*min\(50vw,\s*600px\);[\s\S]*pointer-events:\s*none;/s);
