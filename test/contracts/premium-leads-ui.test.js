...
test('premium leads page bootstraps leads before async refresh starts', () => {
  ...
  assert.doesNotMatch(pageSource, /assets\/premium-personal-assignment-filter\.css/);
  assert.doesNotMatch(pageSource, /assets\/premium-personal-assignment-filter\.js/);
  assert.doesNotMatch(pageSource, /id="onlyMyAssignmentsLeadsToggle"/);
  assert.doesNotMatch(pageSource, /data-only-my-assignments-toggle/);
  assert.doesNotMatch(pageSource, /Enkel mijn toewijzingen bekijken/);
  ...
  assert.match(pageSource, /window\.__softoraLeadsPageCount = currentLeadCount;/);
  assert.doesNotMatch(pageSource, /<script src="assets\/premium-personal-assignment-pages\.js/);
