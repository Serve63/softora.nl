...
function extractFrame(inputPath, seconds, outputPath) {
  return new Promise((resolve, reject) => {
    execFile(ffmpegPath, ['-y', '-ss', String(seconds), '-i', inputPath, '-frames:v', '1', outputPath], {
      shell: false,
      windowsHide: true,
...
test('echte homepage scrollt vloeiend op 30 fps, wordt H.264 MP4 en houdt het lege videovak vast', { timeout: 180_000 }, async () => {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
    return res.end('<!doctype html><style>html,body{margin:0;background:#000}video{display:block;width:1280px;height:720px}</style><video id="player" controls preload="auto" src="/video.mp4"></video>');
  });
