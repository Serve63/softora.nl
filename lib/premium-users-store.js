...
function createPremiumUsersStore({ config = {}, deps = {} } = {}) {
  ...
  function parseScryptPasswordHash(passwordHash) {
    ...
    if (!/^scrypt:/i.test(rawHash)) return null;
    const [, saltBase64 = '', derivedKeyBase64 = ''] = rawHash.split(':');
    if (!saltBase64 || !derivedKeyBase64) return null;
    try {
      const salt = Buffer.from(saltBase64, 'base64');
      const expectedBuffer = Buffer.from(derivedKeyBase64, 'base64');
      if (!salt.length || !expectedBuffer.length) return null;
      return { salt, expectedBuffer };
    } catch {
      return null;
    }
  }
  ...
  function verifyPasswordHash(password, passwordHash) {
    ...
    const parsedHash = parseScryptPasswordHash(passwordHash);
    ...
    try {
      const derivedBuffer = crypto.scryptSync(
        rawPassword,
        parsedHash.salt,
        parsedHash.expectedBuffer.length
      );
      return crypto.timingSafeEqual(derivedBuffer, parsedHash.expectedBuffer);
    } catch {
      return false;
    }
  }
  ...
  function setUsersCache(users, updatedAt = new Date().toISOString(), source = 'supabase') {
  ...
  function buildBootstrapUsers() {
    const passwordHash = parseScryptPasswordHash(premiumLoginPasswordHash)
      ? normalizeString(premiumLoginPasswordHash)
      : createPasswordHash(premiumLoginPassword);
  ...
  function buildFallbackHydrationResult(options, bootstrapUsers) {
    ...
    return buildUnavailableHydrationResult();
  }
  ...
  async function persistUsersCollection(users, meta = {}) {
    ...
    try {
      ...
      if (client) {
        ...
        persistError = new Error('Supabase client ontbreekt.');
      }

      if (persistError) {
        const fallback = await upsertSupabaseRowViaRest(row);
        if (!fallback.ok) {
          console.error('[PremiumUsers][PersistError]', persistError.message || persistError);
          return { users: usersCache.slice(), source: 'unavailable', updatedAt };
        }
      }
  ...
  async function ensureUsersHydrated(options = {}) {
    ...
    if (usersHydrated && !force && usersSource === 'supabase') {
      return { users: usersCache.slice(), updatedAt: usersUpdatedAt, source: 'supabase' };
    }
    if (usersHydrationPromise) {
      if (!hasCustomReadTimeout) return usersHydrationPromise;
      try {
    ...
    usersHydrationPromise = (async () => {
      try {
        ...
        if (storedUsers.length > 0) {
          setUsersCache(storedUsers, row?.updated_at || new Date().toISOString(), 'supabase');
          return { users: usersCache.slice(), updatedAt: usersUpdatedAt, source: 'supabase' };
        }
