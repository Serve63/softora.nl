...
-- Capture this before any data-ops table is created. The later bootstrap-only
-- drain bridge may never treat an existing legacy mailbox database as fresh.
select pg_catalog.set_config(
  'softora.mailbox_uid_fresh_bootstrap',
  case when
    pg_catalog.to_regclass('public.softora_mailbox_campaign_consistency') is null
    and pg_catalog.to_regclass('public.softora_mailbox_sync_state') is null
    and pg_catalog.to_regclass('public.softora_mailbox_messages') is null
    and pg_catalog.to_regclass('public.softora_mailbox_campaign_mutations') is null
  then '1' else '0' end,
  false
);
...
-- A fresh full-schema bootstrap has no running mailbox writers. Move its new
-- singleton through the same forward-only protocol states, with the mandatory
-- drain floor already elapsed, before installing the v2 end state below.
do $mailbox_uid_generation_bootstrap_drain$
declare
  v_now timestamptz := pg_catalog.clock_timestamp();
begin
  if coalesce(pg_catalog.current_setting(
    ...
    if exists (select 1 from public.softora_mailbox_sync_state)
      or exists (select 1 from public.softora_mailbox_messages)
      or exists (select 1 from public.softora_mailbox_campaign_mutations)
      or (select pg_catalog.count(*) from public.softora_mailbox_campaign_consistency) <> 1
      or not exists (
        select 1
        from public.softora_mailbox_campaign_consistency as consistency
        where consistency.scope = 'campaign'
          and consistency.content_version = 0
          and consistency.uid_generation_protocol = 'legacy'
      ) then
      raise exception using errcode = '55000',
        message = 'MAILBOX_UID_FRESH_BOOTSTRAP_NOT_EMPTY';
    end if;
    perform pg_catalog.pg_advisory_xact_lock(824031, 3);
    perform pg_catalog.set_config(
      'softora.mailbox_uid_protocol_transition', '1', true
    ...
    set uid_generation_protocol = 'draining',
        uid_generation_protocol_changed_at = v_now,
        uid_generation_drain_started_at = v_now - interval '3 minutes',
        uid_generation_drain_ready_at = v_now,
        updated_at = v_now
    where consistency.scope = 'campaign'
      and consistency.uid_generation_protocol = 'legacy';
  end if;
  perform pg_catalog.set_config(
    'softora.mailbox_uid_fresh_bootstrap', '', false
  );
end;
$mailbox_uid_generation_bootstrap_drain$;
...
create table if not exists public.softora_mailbox_uid_generation_commits (
  ...
  operation text not null check (operation in ('commit', 'fail')),
