-- Keep outreach discovery within the interactive request budget. The first
-- outreach predicate performed a full participant extraction scan for every
-- search candidate. Index exact participants once and apply the word-prefix
-- boundary directly to the stored normalized search document.

create index if not exists softora_mailbox_messages_participants_active_idx
on public.softora_mailbox_messages using gin (
  public.softora_mailbox_message_participants(sender_email, recipients_text, payload)
)
      ...
      and (
        exists (
          ...
          where guard.key_type = 'email'
            and guard.key_value = p.contact_email
...
as $function$
  with params as (
    ...
    select
      ...
      case
        ...
        when public.softora_mailbox_search_word_prefix(pg_catalog.concat_ws(
          ' ', candidates.recipients_text, candidates.payload->>'toDisplay',
          candidates.payload->>'cc', candidates.payload->>'bcc', candidates.payload->>'replyTo'
        ), p.query) then 550
    ...
    cross join lateral (
      ...
      from pg_catalog.unnest(candidates.participant_emails) as participants(participant)
