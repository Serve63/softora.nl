-- Cheap indexed hints narrow the scan; the existing full campaign-proof gate still decides eligibility.
create function public.softora_mailbox_ai_has_campaign_hint(m public.softora_mailbox_messages)
returns boolean language sql stable security invoker set search_path = '' as $$
  select coalesce(
    exists (select 1 from public.softora_mailbox_campaign_lineage_members x
      where x.message_key=m.message_key and x.account_email=m.account_email)
    or exists (select 1 from public.softora_mailbox_campaign_lineage_roots x
      where x.message_key=m.message_key and x.account_email=m.account_email)
    or nullif(m.in_reply_to,'') is not null
    or nullif(m.references_text,'') is not null
    or m.payload->>'originalCampaignOutbound'='true'
    or (m.folder='instantly' and m.payload->>'provider'='instantly'
      and nullif(m.payload->>'providerThreadId','') is not null), false);
$$;
revoke all on function public.softora_mailbox_ai_has_campaign_hint(public.softora_mailbox_messages)
  from public, anon, authenticated;
grant execute on function public.softora_mailbox_ai_has_campaign_hint(public.softora_mailbox_messages)
  to service_role;
