-- Retry proven campaign failures within the existing approved budget.
-- Keep settled and uncertain costs from every earlier attempt.

alter table public.softora_mailbox_ai_presentations
  add column prior_charged_micro_usd bigint not null default 0
    check (prior_charged_micro_usd >= 0);
alter table public.softora_mailbox_ai_presentations
  drop constraint softora_mailbox_ai_presentations_attempt_count_check;
alter table public.softora_mailbox_ai_presentations
  add constraint softora_mailbox_ai_presentations_attempt_count_check
  ...
  with candidates as (
    select p.id, p.status, p.claim_reserved_micro_usd, p.charged_micro_usd,
      p.attempt_count, (p.attempt_count < 2 or p.attempt_count = 2 and
        (p.status = 'running' or p.usage->>'errorCode' = 'MAILBOX_AI_WORKER_LOST')) retryable
    ...
    where p.version = 'mailbox-luna-v1' and (
      ...
      or (p.status = 'failed' and p.attempt_count < 3
        ...
        and ((p.attempt_count < 2 and p.usage->>'errorCode' in
          ...
          or (p.attempt_count = 2 and p.usage->>'errorCode' = 'MAILBOX_AI_WORKER_LOST')
  ...
  select p.* into job from public.softora_mailbox_ai_presentations p
    ...
    where p.status='queued' and p.version='mailbox-luna-v1'
      and (p.attempt_count < 2 or (p.attempt_count = 2
        and p.usage->>'errorCode' = 'MAILBOX_AI_WORKER_LOST'))
