-- Derive unused inventory from the destination, not only the producer's stale
-- transfer flag. Historical/tombstoned imports and known email aliases remain
-- exclusions: deleting a customer must not make an old import appear new.
-- This is a read-only projection; no source data or recipient guards are changed.
...
comment on view public.softora_kvk_unused_company_directory is
  'Unused usable KVK rows, excluding existing/historical premium imports by KVK, scraper source ID or exact normalized email/alias; not a send-eligibility decision.';
