-- Counts are shared settings; changing a count never starts a worker.
alter table public.softora_kvk_api_budget
  add column if not exists searcher_count integer not null default 1 check (searcher_count between 1 and 10),
  add column if not exists controller_count integer not null default 1 check (controller_count between 1 and 10);

create index if not exists softora_kvk_api_pending_role
  on public.softora_kvk_api_budget_requests (worker_role) where state = 'reserved';
